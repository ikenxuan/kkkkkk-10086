...
const registryCli = join(repositoryRoot, 'src', 'module', 'tooling', 'react-template', 'registry-cli.ts')
const cleanerCli = join(repositoryRoot, 'src', 'module', 'tooling', 'react-template', 'build-cleaner-cli.ts')
...
describe('template tooling CLI contract', () => {
  it('runs every TypeScript-backed template tool through Node 22 type stripping', () => {
    expect(packageJson.scripts?.['template:sync']).toBe(
      'node --experimental-strip-types src/module/tooling/react-template/registry-cli.ts'
    )
    expect(packageJson.scripts?.['template:check']).toBe(
      'node --experimental-strip-types src/module/tooling/react-template/registry-cli.ts --check'
    )
    expect(packageJson.scripts?.['clean:template']).toBe(
      'node --experimental-strip-types src/module/tooling/react-template/build-cleaner-cli.ts'
    )
    expect(packageJson.scripts?.lint).not.toContain('scripts')
    expect(packageJson.scripts?.fix).not.toContain('scripts')
    expect(existsSync(join(repositoryRoot, 'scripts', 'generate-template-registry.mjs'))).toBe(false)
    expect(existsSync(join(repositoryRoot, 'scripts', 'clean-template-build.mjs'))).toBe(false)
  })

  it('does not depend on art-template or the removed raw bridge', () => {
    expect(packageJson.devDependencies?.['art-template']).toBeUndefined()
    expect(existsSync(join(repositoryRoot, 'resources', 'react-template', 'bridge.html'))).toBe(false)
  })
