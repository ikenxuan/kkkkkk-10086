...
describe('Bilibili one_video media tasks', () => {
  ...
  it('starts the information and video branches concurrently, then handles comments', async () => {
    ...
    try {
      ...
      expect(mocks.events).not.toContain('comment:fetch')

      posterGate.resolve()
      await vi.waitFor(() => expect(mocks.events).toContain('poster:end'))
      expect(mocks.events).not.toContain('comment:fetch')

      videoGate.resolve()
      await expect(execution).resolves.not.toBe(false)
      expect(mocks.events.indexOf('comment:fetch')).toBeGreaterThan(mocks.events.indexOf('video:end'))
  ...
  it('routes two branch failures through the existing observable error path', async () => {
    mocks.config.bilibili.sendContent = ['info', 'video']
    const posterError = new Error('poster failed')
    const videoError = new Error('video failed')
    const { subject, getvideo } = createSubject({ posterError, videoError })

    await expect(subject.RESOURCES({ type: 'one_video', bvid: 'BV1test', p: 1 })).resolves.toBe(false)
    expect(getvideo).toHaveBeenCalledOnce()
    expect(mocks.logger.error).toHaveBeenCalledTimes(2)
    expect(mocks.logger.warn).toHaveBeenCalledWith(expect.stringContaining('Bilibili解析错误'))
  })
})
