...
describe('buildDouyinFfmpegPlan', () => {
  const baseInput = {
    videoPath: 'C:\\media files\\source.mp4',
    outputPath: 'C:\\media files\\output.mp4',
    assPath: 'C:\\media files\\source_danmaku.ass',
    filterScriptPath: 'C:\\media files\\douyin-filter.txt',
    width: 1080,
    height: 1920,
    scrollTime: 8,
    encoder: 'libx264'
  ...
  it('uses relative paths from the output directory for an ordinary subtitles filter', () => {
    ...
    expect(plan.command).not.toContain('C:\\media files')
  ...
  it('deduplicates PNG inputs and moves the timed graph into a temporary filter script', () => {
    const overlay: DanmakuOverlay = {
      pngPath: 'C:\\media files\\strip.png',
      startTime: 1000,
      endTime: 9000,
      y: 12,
      moveW: 200,
  ...
  it('keeps a large overlay graph out of the Windows command line', () => {
    const overlays: DanmakuOverlay[] = Array.from({ length: 400 }, (_, index) => ({
      pngPath: `C:\\media files\\strip-${index % 4}.png`,
      startTime: index * 100,
