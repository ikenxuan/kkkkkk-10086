...
interface GroupListProvider {
  /** 群列表 Map。键可能是 number（icqq）也可能是 string（QQBot 等） */
  gl?: HostGroupMap
}
 ...
 */
const createGroupNameResolver = (e: MessageEvent): ((groupId: string) => string) => {
  ...
  // 直接报 TS2589「类型实例化过深」。
  const hostBot = (globalThis as unknown as { Bot?: GroupListProvider }).Bot
  const maps = [(e.bot as GroupListProvider | undefined)?.gl, hostBot?.gl]
    .filter((map): map is HostGroupMap => map instanceof Map)
  ...
  return (groupId: string): string => {
     ...
     */
    if (groupId && groupId === eventGroupId && eventGroupName) return eventGroupName

    // 宿主 `pickGroup()` 就是这么归一化群号的（`Number(group_id) || group_id`）：
    // icqq 的 gl 键是 number，QQBot 的是 string，两种都得试。
    const keys: Array<string | number> = [groupId, Number(groupId) || groupId]
