 ...
 */
const section = (value: unknown): Record<string, unknown> => isRecord(value) ? value : {}
 ...
 * @returns 映射后的配置对象
 */
function mapPushConfig (prefix: string, pushConfig: unknown): Record<string, string> {
  if (!isRecord(pushConfig)) return {}
 ...
 * @returns 映射后的配置对象
 */
function mapProxyConfig (proxyConfig: unknown): Record<string, string> {
  if (!isRecord(proxyConfig)) return {}
 ...
 */
const getStatus = (value: unknown, isCookie = false, isSensitive = false, isQuality = false): string => {
