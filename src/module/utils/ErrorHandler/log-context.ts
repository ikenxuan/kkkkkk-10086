...
const appendLog = (level: CapturedLogLevel, args: unknown[]): void => {
  ...
  if (!context?.active || context.entries.length >= context.maxEntries) return

  try {
    const message = formatArguments(args)
    const raw = `[${formatTimestamp()}][${level}] ${message}`
    const bytes = Buffer.byteLength(raw, 'utf8')
    if (context.bytes + bytes > context.maxBytes) return
...
export const createLogContext = (options: { maxEntries?: number; maxBytes?: number } = {}): ContextLogger => {
  ...
  return {
    ...
    logs: (): CapturedLogEntry[] => state.entries.slice(),
    destroy: (): void => {
      state.active = false
    }
  }
