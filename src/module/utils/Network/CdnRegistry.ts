...
/** 一次失败的性质。全都会让主机进惩罚期，区分开只为了让诊断快照能说清原因。 */
export type CdnFailureKind =
  /** 403 / 401：节点拒绝服务（鉴权、Referer 门、地域限制） */
  | 'blocked'
  /** 404 / 410：这个节点上没有这份资源（镜像还没同步到） */
  | 'missing'
  /** 持续低速，被限速看守判定 */
  | 'slow'
  /** 连不上、握手失败、读超时 */
...
interface HostHealth {
  failures: number
  lastKind: CdnFailureKind | null
  /** 惩罚到期时刻（`Date.now()` 纪元），0 表示不在惩罚期 */
...
/** 单个主机的健康快照，给运行诊断卡消费。 */
export interface CdnHostSnapshot {
  host: string
  failures: number
  lastKind: CdnFailureKind | null
  /** 还剩多少毫秒的惩罚期，0 表示当前可用 */
...
/** 只读快照。形状对齐 `getDownloadBudgetSnapshot()`：只给数字，不给能反过来改状态的句柄。 */
export interface CdnRegistrySnapshot {
  /** 记着候选清单的资源数 */
  resources: number
  /** 见过的主机数 */
  hosts: number
...
const trim = <K, V>(map: Map<K, V>): void => {
...
const isPenalized = (host: string, now: number): boolean => {
  const health = hosts.get(host)
  if (!health) return false
  if (health.penalizedUntil <= now) return false
  return true
}
 ...
 */
export const orderCdnCandidates = (urls: readonly unknown[], now = Date.now()): string[] => {
  const seen = new Set<string>()
  const healthy: string[] = []
  const penalized: string[] = []
  for (const url of urls) {
    if (!isDownloadableUrl(url) || seen.has(url)) continue
    seen.add(url)
    const host = readHostname(url)
    if (host !== null && isPenalized(host, now)) penalized.push(url)
 ...
 * @param now 记账时刻，测试用
 */
export const rememberCdnCandidates = (
  resource: string,
  urls: readonly unknown[],
  now = Date.now()
): string[] => {
  const ordered = orderCdnCandidates(urls, now)
  if (resource.length === 0 || ordered.length === 0) return ordered
 ...
 * @param now 判定时刻，测试用
 */
export const getCdnCandidates = (resource: string, now = Date.now()): string[] => {
 ...
 * @param now 判定时刻，测试用
 */
export const resolveCdnCandidates = (
  resource: string,
  urls: readonly unknown[],
  now = Date.now()
): string[] => {
  const fresh = orderCdnCandidates(urls, now)
  if (resource.length === 0) return fresh
 ...
 * @param now 记账时刻，测试用
 */
export const reportCdnFailure = (url: string, kind: CdnFailureKind, now = Date.now()): void => {
  const host = readHostname(url)
  if (host === null) return
 ...
 * @param url 成功的下载地址
 */
export const reportCdnSuccess = (url: string): void => {
  const host = readHostname(url)
  if (host === null) return
  const existing = hosts.get(host)
  if (!existing) return
...
export const isCdnHostPenalized = (url: string, now = Date.now()): boolean => {
  const host = readHostname(url)
 ...
 * @param now 判定时刻，测试用
 */
export const getCdnRegistrySnapshot = (now = Date.now()): CdnRegistrySnapshot => {
  for (const [resource, entry] of candidates) {
    if (entry.expiresAt <= now) candidates.delete(resource)
  }
  const penalized: CdnHostSnapshot[] = []
  for (const [host, health] of hosts) {
    if (health.penalizedUntil <= now) continue
    penalized.push({
      host,
...
/** 清空地址簿。测试用。 */
export const resetCdnRegistry = (): void => {
  hosts.clear()
  candidates.clear()
}
 ...
 */
export const classifyCdnFailure = (error: unknown): CdnFailureKind | null => {
