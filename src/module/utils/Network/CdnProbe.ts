...
interface CachedProbe {
  result: CdnProbeResult
  expiresAt: number
}

const cache = new Map<string, CachedProbe>()

const readHost = (url: string): string | null => {
  try {
...
const remember = (host: string, result: CdnProbeResult, now: number): void => {
  cache.delete(host)
...
const recall = (host: string, now: number): CdnProbeResult | undefined => {
 ...
 */
export const probeCdnUrl = async (url: string, options: ProbeOptions = {}): Promise<CdnProbeResult> => {
 ...
 */
const compareProbes = (a: CdnProbeResult, b: CdnProbeResult): number => {
  if (a.ok !== b.ok) return a.ok ? -1 : 1
  if (!a.ok) return b.ttfbMs - a.ttfbMs
  const faster = Math.max(a.bytesPerSecond, b.bytesPerSecond)
  const gap = Math.abs(a.bytesPerSecond - b.bytesPerSecond)
  if (faster > 0 && gap / faster > 0.05) return b.bytesPerSecond - a.bytesPerSecond
  return a.ttfbMs - b.ttfbMs
}

export interface ProbeAndOrderOptions extends ProbeOptions {
  /** 最多探几个地址。默认 4 —— 再多就是拿时间换一个用不上的排名 */
  limit?: number
  /** 判定时刻，测试用 */
  now?: number
}
 ...
 */
export const probeAndOrderCdnUrls = async (
  urls: readonly string[],
  options: ProbeAndOrderOptions = {}
): Promise<string[]> => {
  const limit = options.limit ?? 4
  const now = options.now ?? Date.now()
  const head = urls.slice(0, Math.max(0, limit))
  const tail = urls.slice(Math.max(0, limit))
  if (head.length <= 1) return [...urls]

  const results = await Promise.all(head.map(async url => {
    const host = readHost(url)
    if (host === null) return { url, host: '', ok: false, ttfbMs: 0, bytesPerSecond: 0, error: 'bad url' }
...
/** 只读快照，给运行诊断卡消费。 */
export interface CdnProbeSnapshot {
  hosts: number
  entries: readonly {
    host: string
    ok: boolean
 ...
 * @param now 判定时刻，测试用
 */
export const getCdnProbeSnapshot = (now = Date.now()): CdnProbeSnapshot => {
  const entries: CdnProbeSnapshot['entries'][number][] = []
...
/** 清空测速缓存。测试用。 */
export const resetCdnProbe = (): void => {
  cache.clear()
}
