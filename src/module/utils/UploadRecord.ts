...
type ProtobufValue = {
  [key: number]: ProtobufValue | ProtobufValue[] | number | string | Buffer | undefined
  toHex?: () => string
  toBuffer?: () => Buffer
}
...
type ErrorConstructorLike = new (...args: unknown[]) => Error

type Core = {
  pb: {
    encode: (data: unknown) => Uint8Array | undefined
    decode: (data: unknown) => ProtobufValue
  }
  ApiRejection: ErrorConstructorLike
}

type UploadRecordBot = Omit<NonNullable<MessageEvent['bot']>, 'sendUni'> & {
  uin: string | number
  sendUni?: (service: string, body: Uint8Array | undefined) => Promise<unknown>
}

type UploadRecordEvent = Omit<MessageEvent, 'bot'> & { bot: UploadRecordBot }
type RecordSource = string | Buffer
...
type AudioTimeResult = {
  code: number
  buffer?: Buffer
  data?: AudioTime
}

type PttBufferResult = {
  buffer: Buffer
  time?: AudioTime
}

type ErrorCode = {
  ClientNotOnline: number
  PacketTimeout: number
  UserNotExists: number
  GroupNotJoined: number
  MemberNotExists: number
  MessageBuilderError: number
  RiskMessageError: number
  SensitiveWordsError: number
  HighwayTimeout: number
  HighwayNetworkError: number
  NoUploadChannel: number
  HighwayFileTypeError: number
  UnsafeFile: number
  OfflineFileNotExists: number
  GroupFileNotExists: number
  FFmpegVideoThumbError: number
  FFmpegPttTransError: number
}

type Errors = {
  ErrorCode: ErrorCode
  drop: (code: number, message?: string) => never
}
 ...
 */
errors.drop = function (code: number, message?: string): never {
 ...
 * @returns {Promise<ReturnType<typeof segment.record> | MessageElement>} 返回语音消息对象
 */
async function UploadRecord (
  e: UploadRecordEvent,
  record_url: RecordSource,
  seconds = 0,
  transcoding = true,
  brief = ''
  ...
  try {
    ...
    const body = core.pb.encode({
      ...
      5: {
        ...
        3: 0,
        4: hash,
        5: recordSize > 10485759 ? 10485759 : recordSize,
        6: hash,
        7: 5,
        8: 9,
        9: 4,
        ...
        11: 0,
        12: seconds && seconds < 300 ? seconds : 299,
        13: 1,
        14: codec,
        15: 1
      }
    ...
    const rsp = core.pb.decode(payload)[5] as PttResponse
    if (rsp[2]) errors.drop(rsp[2], rsp[3])

    const ipValue = rsp[5]
    const portValue = rsp[6]
    const ip = (Array.isArray(ipValue) ? (ipValue[0] || ipValue) : ipValue) as number
    const port = (Array.isArray(portValue) ? (portValue[0] || portValue) : portValue) as number
    const params = {
      ver: 4679,
      ukey: rsp[7].toHex(),
    ...
    await fetch(`http://${int32ip2str(ip)}:${port}/?${querystring.stringify(params)}`, {
      ...
      body: result.buffer as unknown as BodyInit
    })
    ...
    const b = core.pb.encode({
      ...
      6: recordSize > 10485759 ? 10485759 : recordSize,
      11: 1,
      18: fid,
      19: seconds && seconds < 300 ? seconds : 299,
      29: codec,
 ...
 */
async function prepareAudioFile (file: RecordSource): Promise<string> {
  ...
  try {
    ...
    if (Buffer.isBuffer(file) || (typeof file === 'string' && file.startsWith('base64://'))) {
      ...
      try {
        await network.downloadStream((downloaded: number, total: number) => {
          if (total > 0) {
 ...
 */
async function getPttBuffer (filePath: string, transcoding = true): Promise<PttBufferResult> {
 ...
 */
async function audioTrans (file: string): Promise<Buffer | false> {
  try {
 ...
 */
async function audioTransFallback (file: string): Promise<Buffer | false> {
 ...
 */
async function read7Bytes (file: string): Promise<Buffer> {
 ...
 */
function int32ip2str (ip: number | string): string {
