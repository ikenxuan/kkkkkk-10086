    ...
    try {
      ...
      switch (this.type) {
        case 'one_work': {
          ...
          const getEmojiList = (): Promise<DouyinEmojiInfo[]> => {
            emojiListPromise ??= (async () => {
              try {
                ...
                const fromApi = Emoji(emojiData.data).filter(
                ...
                return mergeDouyinEmojiList(fromApi) as DouyinEmojiInfo[]
              } catch (error) {
           ...
           */
          const g_title: string | undefined = isVideo
        ...
        case 'music_work': {
          ...
          if (!music) throw new Error('音乐数据缺少 music_info')
