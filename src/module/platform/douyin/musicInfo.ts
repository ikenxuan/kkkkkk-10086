...
import { buildAmagiRequestConfig, douyinGuest } from '@/module/utils/amagiClient'
import { isRecord } from '@/module/utils/record'

import type { DouyinMusicInfo } from './types.js'

/** 同 `musicUrl.ts`：裸写 `logger?.debug?.()` 会 ReferenceError，所以显式窄化一次 */
type HostLogger = {
  debug?: (message: string, ...rest: unknown[]) => void
  info?: (message: string) => void
  mark?: (message: string) => void
}
const hostLogger = (globalThis as { logger?: HostLogger }).logger

export interface GuestMusicFetchOptions {
  fetchGuestMusicInfo?: (musicId: string) => Promise<unknown>
}

export interface ResolveDouyinMusicOptions extends GuestMusicFetchOptions {
  fetchGuestMusicAwemeList?: (musicId: string, number: number) => Promise<unknown>
  /** 取源作品用，由调用方带上 cookie 与 requestConfig；不传就只拿原声本体 */
  parseWork?: (awemeId: string) => Promise<unknown>
  /** 最多试几个源作品，默认 3 —— 目的是拿到 mp3，不是遍历作品 */
  maxWorkAttempts?: number
  count?: number
}

export interface FetchDouyinMusicDetailOptions {
  /** 打 `music/detail`，由调用方带上 cookie 与 requestConfig；不传就直接返回 `null` */
  fetchMusicInfo?: (musicId: string) => Promise<unknown>
}

export interface ResolvedDouyinMusic {
  music_info: DouyinMusicInfo
  source: 'music_detail' | 'guest' | 'guest+work'
  /** 提供 mp3 的那个作品；没取到时是空串 */
  sourceAwemeId: string
}

/** 从 amagi 的原样信封里剥一个字段，两层 `data` 都看（不同路径的嵌套层数不一样） */
const readEnvelope = (response: unknown, field: string): unknown => {
...
const readMusicExtra = (music: unknown): Record<string, unknown> => {
  if (!isRecord(music) || typeof music.extra !== 'string') return {}
  try {
    const parsed: unknown = JSON.parse(music.extra)
 ...
 */
export const readDouyinMusicSourceAwemeId = (music: unknown): string => {
  if (!isRecord(music) || typeof music.extra !== 'string') return ''
  return /"extract_item_id"\s*:\s*(\d+)/.exec(music.extra)?.[1] ?? ''
}

/** 游客接口取原声本体，失败返回 `null` */
export const fetchGuestDouyinMusicInfo = async (
  musicId: string | undefined | null,
  options: GuestMusicFetchOptions = {}
): Promise<Record<string, unknown> | null> => {
  const id = String(musicId ?? '').trim()
  if (!id) return null

  const request = options.fetchGuestMusicInfo ?? (async (mid: string) => {
 ...
 */
export const fetchGuestDouyinMusicAwemeIds = async (
  musicId: string | undefined | null,
  options: ResolveDouyinMusicOptions = {}
): Promise<string[]> => {
  const id = String(musicId ?? '').trim()
  if (!id) return []
  ...
  const request = options.fetchGuestMusicAwemeList ?? (async (mid: string, number: number) => {
  ...
  try {
    ...
    return list
      .map((item: unknown) => (isRecord(item) && typeof item.aweme_id === 'string' ? item.aweme_id : ''))
      .filter(Boolean)
  } catch (error) {
 ...
 */
export const getDouyinMusicianName = (music: unknown): string => {
  if (!isRecord(music)) return ''
  const extra = readMusicExtra(music)
  return firstNonEmptyString(
    music.original_musician_display_name,
    music.owner_nickname,
    extra.owner_nickname,
    music.author
  )
}
 ...
 */
const readMusicUseCount = (guestExtra: Record<string, unknown>): number | null => {
...
export const mergeDouyinMusicInfo = (guestInfo: unknown, workMusic?: unknown): DouyinMusicInfo => {
  ...
  if (nickname) merged.owner_nickname = nickname

  // 超集，唯一对不上的是 `id`：游客接口不下发，且它本来就是丢了精度那份
  return merged as unknown as DouyinMusicInfo
}

/** 作品上的 music 对象。带 `play_url` 才算有效，排掉游客作品列表里那种空壳 music */
const readWorkMusic = (response: unknown): Record<string, unknown> | undefined => {
 ...
 */
export const resolveDouyinMusicInfo = async (
  musicId: string | undefined | null,
  deps: ResolveDouyinMusicOptions = {}
): Promise<ResolvedDouyinMusic | null> => {
  const id = String(musicId ?? '').trim()
  if (!id) return null
  ...
  let workMusic: Record<string, unknown> | undefined
  let sourceAwemeId = ''

  const tryWork = async (awemeId: string): Promise<boolean> => {
    if (!parseWork || !awemeId || tried.has(awemeId) || tried.size >= maxAttempts) return false
    tried.add(awemeId)
    try {
  ...
  if (parseWork) {
    ...
    if (!await tryWork(readDouyinMusicSourceAwemeId(guest))) {
      for (const awemeId of await fetchGuestDouyinMusicAwemeIds(id, deps)) {
        if (await tryWork(awemeId) || tried.size >= maxAttempts) break
      }
    }
  }
  ...
  emitLog?.call(
    hostLogger,
    `[抖音原声] 游客接口取数成功 music_id=${id} 源作品=${sourceAwemeId || '无'} 音频=${workMusic ? '有' : '无'}`
  )
 ...
 */
const readMusicDetail = (response: unknown): Record<string, unknown> | undefined => {
  const music = readEnvelope(response, 'music_info')
  if (!isRecord(music)) return undefined
 ...
 */
export const fetchDouyinMusicDetail = async (
  musicId: string | undefined | null,
  deps: FetchDouyinMusicDetailOptions = {}
): Promise<ResolvedDouyinMusic | null> => {
  const id = String(musicId ?? '').trim()
  const fetchMusicInfo = deps.fetchMusicInfo
  if (!id || !fetchMusicInfo) return null
  ...
  emitLog?.call(
    hostLogger,
    `[抖音原声] music/detail 取数成功 music_id=${id} 字段=${Object.keys(music).length} 音频=${isRecord(music.play_url) ? '有' : '无'}`
  )
  return { music_info: music as unknown as DouyinMusicInfo, source: 'music_detail', sourceAwemeId: '' }
}
