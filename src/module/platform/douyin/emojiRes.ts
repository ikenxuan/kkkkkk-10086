...
/** 一条补充表情，形状与 `Emoji()` 的产物一致，可直接并进表情表 */
export interface EmojiSupplementItem {
  /** 带方括号的名字，例如 `[登场]` */
  name: string
  /** data URI —— 渲染走 puppeteer 截本地 HTML，`file://` 的协议白名单行为不好保证 */
  url: string
}

interface SupplementCache {
  /** 资源包 md5，也是「要不要重下」的唯一判据 */
  md5: string
  items: EmojiSupplementItem[]
}

const cacheFile = (): string => path.join(DataPath, 'douyin-emoji', 'supplement.json')
 ...
 */
const readZip = (buf: Buffer): Map<string, () => Buffer> => {
  let eocd = -1
...
const readMeta = (response: unknown): { md5: string; url: string } | null => {
...
const requestEmojiResourceMeta = async (): Promise<unknown> => {
  const call = douyinGuest('fetchEmojiResourceMeta')
  if (!call) throw new Error('装的 amagi 没有 fetchEmojiResourceMeta')
  return await call({}, '', buildAmagiRequestConfig())
}

export interface SyncEmojiResourceResult {
  md5: string
  /** 存进缓存的限时表情条数 */
  count: number
...
export const syncDouyinEmojiResource = async (
  requestMeta: () => Promise<unknown> = requestEmojiResourceMeta
): Promise<SyncEmojiResourceResult | null> => {
  const file = cacheFile()

  let meta: { md5: string; url: string } | null
  try {
  ...
  const cached = readJson<SupplementCache>(file)
  ...
  try {
    ...
    if (got !== meta.md5) throw new Error(`md5 不匹配（期望 ${meta.md5} 实得 ${got}）`)

    const entries = readZip(buf)
    const info = entries.get('info.json')
    if (!info) throw new Error('包里没有 info.json')
    const parsed = JSON.parse(info().toString('utf8')) as unknown
    const stickers = isRecord(parsed) && Array.isArray(parsed.stickers) ? parsed.stickers : []

    const items: EmojiSupplementItem[] = []
    for (const s of stickers) {
      // 只要限时的：web 那份必然覆盖全部常驻表情，缺口只可能在限时里
      if (!isRecord(s) || s.time_limited !== 1) continue
    ...
    if (!items.length) throw new Error('包里没有 time_limited=1 的条目')

    fs.mkdirSync(path.dirname(file), { recursive: true })
    fs.writeFileSync(file, JSON.stringify({ md5: meta.md5, items } satisfies SupplementCache))
 ...
 */
let pendingSync: Promise<SyncEmojiResourceResult | null> | undefined
export const syncDouyinEmojiResourceOnce = (): Promise<SyncEmojiResourceResult | null> => {
  pendingSync ??= syncDouyinEmojiResource().catch(() => null)
  return pendingSync
}

/** 读缓存，产出 `known` 里没有的那批补充项 */
export const loadDouyinEmojiSupplement = (known: ReadonlySet<string>): EmojiSupplementItem[] =>
  (readJson<SupplementCache>(cacheFile())?.items ?? []).filter(item => !known.has(item.name))

/** 把补充项并进表情表。接口已有的一律不覆盖 —— 哪天 web 那张快照更新了，自动用官方的 */
export const mergeDouyinEmojiList = <T extends { name: string; url?: string }>(
  base: readonly T[]
): Array<T | EmojiSupplementItem> => {
