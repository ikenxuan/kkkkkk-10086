...
export interface ResolvedDouyinUser {
  sec_uid: string
  nickname: string
  unique_id: string
  short_id?: string
...
export interface ResolveDouyinUserOptions {
  fetchUserInfo?: (uniqueId: string) => Promise<unknown>
}

/** amagi 回的是原样信封，两层 `data` 都看一眼（不同路径的嵌套层数不一样） */
const readUserInfo = (response: unknown): Record<string, unknown> | undefined => {
...
export const resolveDouyinUserByShortId = async (
  shortId: string | undefined | null,
  options: ResolveDouyinUserOptions = {}
): Promise<ResolvedDouyinUser | null> => {
  const id = String(shortId ?? '').trim()
  if (!id) return null

  const fetchUserInfo = options.fetchUserInfo ?? (async (uniqueId: string) => {
  ...
  if (returned && returned.toLowerCase() !== id.toLowerCase()) return null
