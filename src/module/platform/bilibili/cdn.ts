...
const upos = (code: string): string => UPOS_TEMPLATE.replace('{}', code)
 ...
 */
export const BILIBILI_UPOS_MIRRORS: readonly string[] = Object.freeze([
  upos('coso1'),
  upos('cos'),
  upos('alio1'),
  upos('hwo1'),
...
export const isBilibiliProvincialUrl = (url: string): boolean => {
  try {
 ...
 */
export const rewriteToUposMirror = (url: string, mirror: string): string | null => {
  try {
 ...
 */
export const rewriteToUpstreamSource = (url: string): string | null => {
  try {
    const parsed = new URL(url)
    const source = parsed.searchParams.get('xy_usource')
    if (!source) return null
    // 参数值必须像个主机名。这是外部数据，直接往 hostname 上塞会把畸形值带进请求。
    if (!/^[a-z0-9.-]+\.[a-z]{2,}$/i.test(source)) return null
    parsed.protocol = 'https:'
 ...
 *   全套上去会把重试预算耗在猜地址上
 */
export const expandBilibiliCdnCandidates = (
  urls: readonly string[],
  mirrors: readonly string[] = BILIBILI_UPOS_MIRRORS,
  mirrorLimit = 2
): string[] => {
  const usable: string[] = []
  const provincial: string[] = []
  ...
  for (const url of urls) {
    ...
    if (isBilibiliProvincialUrl(url)) provincial.push(url)
  ...
  if (seed !== undefined) {
    for (const mirror of mirrors.slice(0, Math.max(0, mirrorLimit))) {
      ...
      if (candidate !== null) rewritten.push(candidate)
    }
  }
 ...
 */
export const isUposMirrorUrl = (url: string): boolean => {
  try {
