 ...
 */
export const fallbackAmagiRuntime: PushAmagiRuntime = {
  DynamicType: {
    AV: 'DYNAMIC_TYPE_AV',
 ...
 * 两级都不成才用兜底副本。
 */
export const loadAmagiRuntime = (): PushAmagiRuntime => {
  try {
    return require('@ikenxuan/amagi') as PushAmagiRuntime
  } catch {
    try {
      const axiosEntry = require.resolve('@ikenxuan/amagi/axios')
      return require(resolve(axiosEntry, '../../default/index.cjs')) as PushAmagiRuntime
    } catch {
