...
/**
 * 各平台弹幕原始条目。B站使用 `progress`/`content`/`mode`，
 * 抖音使用 `offset_time`/`time`/`text`，故字段均为可选。
 */
export interface DanmakuItem {
  progress?: number | string
  content?: string
  mode?: number | string
  offset_time?: number | string
  time?: number | string
  text?: string
}
...
export interface DanmakuOptions {
  ...
  danmakuOpacity?: number | string
  danmakuArea?: number | string
  fontName?: string
}
...
type FontSizeName = keyof typeof FONT_SIZE

const isFontSizeName = (value: unknown): value is FontSizeName =>
  value === 'small' || value === 'medium' || value === 'large'

const escapeAss = (text: unknown): string => String(text || '')
  .replace(/\\/g, '\\\\')
  .replace(/\{/g, '\\{')
  .replace(/\}/g, '\\}')
  .replace(/\r?\n/g, ' ')
...
const normalizeFilterPath = (filepath: string): string => filepath
  .replace(/\\/g, '/')
  .replace(/^([A-Za-z]):/, '$1\\:')
  .replace(/'/g, "\\'")
...
const normalizeDanmaku = (source: DanmakuSource, item: DanmakuItem): NormalizedDanmaku => {
  if (source === 'bilibili') {
    return {
...
const generateAss = (
  ...
  const font = isFontSizeName(options.danmakuFontSize) ? FONT_SIZE[options.danmakuFontSize] : FONT_SIZE.medium
 ...
 * @param options 弹幕样式选项
 */
export const burnDanmaku = async (
  source: DanmakuSource,
  videoPath: string,
  danmakuList: DanmakuItem[] | null | undefined,
  outputPath: string,
  options: DanmakuOptions = {}
): Promise<boolean> => {
