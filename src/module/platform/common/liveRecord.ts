...
import { pickDouyinLiveStream } from '@/module/platform/douyin/live'
 ...
 */
const BILIBILI_LIVE_SUFFIX: Record<string, string> = {
  flv: 'flv',
  ts: 'ts',
  fmp4: 'mp4'
}
...
interface LiveSource {
  ...
  /** 落盘后缀，不带点 */
  suffix: string
  /** 录制时要带的请求头。B站的 CDN 校验 Referer，不带就是 403 */
  headers?: Record<string, string>
  /** 文件名主干，已过 `sanitizeFilenameSegment` */
  name: string
  /** 直播间标题，只进提示语 */
  title: string
}
...
/**
 * 取流的结果。失败原因直接带上给用户看的那句话 —— 「录不到」有五六种原因
 ...
 */
const resolveMaxDuration = (configuredSeconds: number | undefined): { ms: number, clamped: boolean } => {
...
const resolveDouyinSource = async (url: string): Promise<LiveSourceResult> => {
  ...
  return {
    ...
    source: {
      ...
      headers: { ...(baseHeaders as Record<string, string>), Referer: 'https://live.douyin.com/' },
 ...
 */
export const recordLiveRoom = async (
  e: CommandEvent,
  ...
  const { ms: maxDurationMs, clamped } = resolveMaxDuration(
    platform === 'douyin' ? Config.douyin.live?.maxDuration : Config.bilibili.live?.maxDuration
  )
  if (clamped) {
  ...
  await e.reply!(
    `开始录制${source.title ? `「${source.title}」` : ''}\n` +
  ...
  if (Config.upload.usefilelimit && Config.upload.filelimit && sizeMB > Config.upload.filelimit) {
    ...
    await e.reply!(
      `录好了 ${sizeMB.toFixed(1)}MB，但超过「视频上传拦截阈值」（${Config.upload.filelimit}MB），已删掉。\n` +
      '把录制时长调小、或者把阈值调大再试'
    )
    return false
  }
