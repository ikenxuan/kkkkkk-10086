 ...
 * 预览路径要读的事件字段。
 *
 * `BaseEvent` 里没有会话号（它只声明 reply / bot / group 那几个），而 `uploadFile`
 * 的入参恰恰是 `BaseEvent`。所以这里在它上面补三个可选字段，而不是换成
 * `MessageEvent` —— 那个接口把 `bot` / `group` 声明成了另一套形状，
 * 两边交起来会让各平台的 `this.e` 在调用点被判成不可赋值。
 */
type LivePreviewEvent = BaseEvent & {
  self_id?: string | number
  group_id?: string | number
  user_id?: string | number
}

/** 队列里的一项：一个房间一项，订阅者可以有多个 */
interface PreviewJob {
  platform: LiveRecordPlatform
  roomKey: string
  roomUrl: string
  /**
   * 运行期的订阅者：事件对象直接留着，录完 `e.reply` 就发出去了。
   *
   * 重启后恢复的那些项这里是空的 —— `e` 序列化不了，那条路走
   * `Bot[selfId].pickGroup/pickFriend`（见 {@link deliverToRow}）。
   */
  events: LivePreviewEvent[]
  /**
   * 这一项的落盘写入链。
   *
   * 落盘是异步的，而认领队列必须是同步的（见 {@link enqueueLivePreview}），
   * 两者一错开就可能出现「录完放掉账本 → 迟到的那条订阅行才写进去」的孤儿行，
   * 下次启动会把它当未完成的项再录一遍。所以每次入队把写入 promise 串在这里，
   * {@link drain} 放账本之前先等它。
   */
...
const queue = new Map<string, PreviewJob>()
 ...
 */
export const livePreviewRoomKey = (platform: LiveRecordPlatform, roomId: string | number): string =>
  `${platform}:${roomId}`
 ...
 */
const readSession = (e: LivePreviewEvent): {
  selfId: string
  ...
  if (groupId) return { selfId, sessionType: 'group', sessionId: groupId }
  if (userId) return { selfId, sessionType: 'private', sessionId: userId }
  return undefined
}
 ...
 */
export const enqueueLivePreview = async (
  e: LivePreviewEvent,
  platform: LiveRecordPlatform,
  ...
  if (!existing) queue.set(roomKey, job)
 ...
 * @param job 队列项
 */
const runJob = async (job: PreviewJob): Promise<void> => {
 ...
 * @param filePath 录好的文件
 */
const deliverToRow = async (row: LivePreviewRow, filePath: string): Promise<void> => {
  const bot = (globalThis as { Bot?: Record<string, unknown> }).Bot?.[row.selfId] as {
    pickGroup?: (id: string) => { sendMsg?: (msg: unknown) => Promise<unknown> }
    pickFriend?: (id: string) => { sendMsg?: (msg: unknown) => Promise<unknown> }
  } | undefined
  if (!bot) {
 ...
 * @returns 重新排队的房间数
 */
export const restoreLivePreviewQueue = async (): Promise<number> => {
  const db = await getLivePreviewDB()
  ...
  for (const row of rows) {
    ...
    if (queue.has(row.roomKey) || active?.roomKey === row.roomKey) continue
    if (queue.size >= MAX_QUEUE_DEPTH) {
      logger.debug('[直播预览] 恢复时队列已满，剩下的行留在账本里等下次启动')
      break
    }
    if (row.platform !== 'douyin' && row.platform !== 'bilibili') continue
...
/** 测试用：清掉内存队列。生产路径不该调它 */
export const __resetLivePreviewQueue = (): void => {
  queue.clear()
  active = null
  draining = null
}
 ...
 */
export const __awaitLivePreviewIdle = async (timeoutMs = 5000): Promise<void> => {
  ...
  for (;;) {
    ...
    if (!current) return
    if (Date.now() > deadline) throw new Error('直播预览队列在超时前没有跑空')
    await current
  }
}
