...
/** 宿主的群成员表：群号 -> （用户号 -> 成员信息）。两层键的类型各适配器不一 */
type HostMemberMap = Map<string | number, Map<string | number, unknown> | undefined>

/** 各宿主的 Bot 对象结构不一致，这里只声明真正读到的成员 */
 ...
 */
const readMemberName = (info: unknown): string => {
...
const shortenUserId = (userId: string): string =>
  userId.length > 14 ? `${userId.slice(0, 6)}…${userId.slice(-4)}` : userId
 ...
 * @param groupId 要查的群号
 */
const createMemberNameResolver = (e: MessageEvent, groupId: string): ((userId: string) => string) => {
  ...
  const hostBot = (globalThis as unknown as { Bot?: MemberListProvider }).Bot

  // 群号也要按宿主 `pickGroup()` 的方式归一化（`Number(id) || id`）：
  // icqq 的 gml 键是 number，QQBot 的是 string，两种都得试。
  const groupKeys: Array<string | number> = [groupId, Number(groupId) || groupId]
  const memberMaps = [(e.bot as MemberListProvider | undefined)?.gml, hostBot?.gml]
    .filter((map): map is HostMemberMap => map instanceof Map)
    .flatMap(map => groupKeys.map(key => map.get(key)))
    .filter((map): map is Map<string | number, unknown> => map instanceof Map)

  return (userId: string): string => {
    // 用户号同样两种键都试，宿主 `pickFriend()` 也是这么归一化的
    const userKeys: Array<string | number> = [userId, Number(userId) || userId]
 ...
 */
export const buildGroupUserRanking = (
  e: MessageEvent,
  groupId: string,
  rows: GroupUserRankingRow[]
): GroupUserRankingEntry[] => {
