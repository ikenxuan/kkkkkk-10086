...
import type { MediaMetricsSummary } from '@/types/database'

/**
 * 契约 `GroupStatisticsData['mediaMetrics']` / `GlobalStatisticsData['mediaMetrics']`
 * 的类型。两张卡共用，定义在 `ktr/template/types/media-metrics.ts`。
 *
 * 这里重新声明而不是从模板树 import：`src/` 侧的 tsconfig 不包含 `ktr/template/`
 * （模板走 tsconfig.template.json / tsconfig.render.json 两个独立程序）。
 * 两边形状不一致时由 `pnpm typecheck:render` 在 Render() 调用点报出来 ——
 * 那是这个仓库校验模板契约的既有机制。
 */
export interface MediaMetricsView {
  ...
  successRate?: number
  platforms: Record<'douyin' | 'bilibili' | 'kuaishou' | 'xiaohongshu', {
    mediaCount: number
    totalDurationMs: number
    durationSamples: number
    averageDurationMs?: number
 ...
 */
export const buildMediaMetricsView = (summary: MediaMetricsSummary): MediaMetricsView | undefined => {
  if (summary.mediaCount === 0 && summary.successCount + summary.failureCount === 0) return undefined
