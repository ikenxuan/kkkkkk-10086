  ...
  async RESOURCES (data) {
    ...
    switch (this.type) {
      case 'one_work': {
        ...
        /** 发送视频 */
        sendvideofile && this.is_mp4 && await this.DownLoadVideo(this.e, { video_url: g_video_url, title: { timestampTitle: `tmp_${Date.now()}.mp4`, originTitle: `${g_title}.mp4` } })
        return true
      }
      case 'user_dynamic': {
      ...
      case 'music_work': {
        ...
        await this.e.reply(
          this.mkMsg(
            ...
            [{ text: '音乐文件', link: MusicData.music_info.play_url.uri }]
          )
        )
          ...
          const img = await Render.render('douyin/live',
            {
              ...
              fans: Common.count(UserInfoData.user.follower_count),
