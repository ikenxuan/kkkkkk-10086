...
import checkuser from './genParams.js'
export default class Bilidata extends Base {
  ...
  async GetData (data) {
    ...
    switch (this.type) {
      case 'bilibilivideo': {
        if (data.id_type === 'aid') data.id = av2bv(data.id)
        const INFODATA = await getBilibiliData('单个视频作品数据', Config.cookies.bilibili, { bvid: data.id })
