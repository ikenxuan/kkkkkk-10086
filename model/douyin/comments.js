...
export async function comments(data, emojidata) {
    ...
    //             // 将emojidata中对应name的url替换到jsonArray中的text中
    //             const imageUrl = `!${item2.name}(${item2.url})`
    //             
    //             // 检查是否存在中括号
    //             if (item1.text.includes('[') && item1.text.includes(']')) {
    //                 // 如果存在，替换现有中括号及其内容
    //                 item1.text = item1.text.replace(/\[[^\]]*\]/g, imageUrl)
    //             } else {
    //                 // 否则，直接替换文本
    //                 item1.text = item1.text.replace(new RegExp(item2.name, 'g'), imageUrl)
    //             }
    //         }
    //     }
    // }
