  ...
  async v1_dy_data(Data, CommentData, is_mp4) {
    ...
    if (is_mp4 === false) {
      ...
      for (let i = 0; i < Data.aweme_detail.images.length; i++) {
        ...
        if (Config.rmmp4 === false) {
          await mkdirs(this._path + `/resources/kkkdownload/images/${g_title}`)
          let path = this._path + `/resources/kkkdownload/images/${g_title}/${i + 1}.png`
    ...
    if (Data.aweme_detail.music) {
      ...
      if (is_mp4 === false && Config.rmmp4 === false && music_url !== undefined) {
        try {
          let path = this._path + `/resources/kkkdownload/images/${g_title}/BGM.mp3`
