...
async function uploadRecord(
	...
	const result = await getPttBuffer(
		record_url,
	...
	const codec = String(buf.slice(0, 7)).includes('SILK')
		? transcoding
			? 1
			: 0
		: 0
	...
	const url =
		`http://${(0, int32ip2str)(ip)}:${port}/?` + querystring.stringify(params)
	const headers = {
...
function md5Stream(readable) {
	return new Promise((resolve, reject) => {
		...
		readable.pipe(
			crypto.createHash('md5').on('error', reject).on('data', resolve)
		)
	})
}
...
function fileHash(filepath) {
	...
	const sha = new Promise((resolve, reject) => {
		readable.on('error', reject)
		readable.pipe(
			crypto.createHash('sha1').on('error', reject).on('data', resolve)
		)
	})
...
;(function (ErrorCode) {
	...
	/** 获取视频中的图片失败 */
	ErrorCode[(ErrorCode['FFmpegVideoThumbError'] = -210)] =
		'FFmpegVideoThumbError'
	/** 音频转换失败 */
