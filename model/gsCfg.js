...
class GsCfg {
  constructor () {
    /** 默认设置 */
    this.defSetPath = './plugins/kkkkkk-10086/defSet/'
    this.defSet = {}

    /** 用户设置 */
    this.configPath = './plugins/kkkkkk-10086/config/'
    this.config = {}
  ...
  cpCfg (app, name) {
    if (!fs.existsSync('./plugins/kkkkkk-10086/config')) {
      fs.mkdirSync('./plugins/kkkkkk-10086/config')
    }

    let set = `./plugins/kkkkkk-10086/config/${app}.${name}.yaml`
    if (!fs.existsSync(set)) {
      fs.copyFileSync(`./plugins/kkkkkk-10086/defSet/${app}/${name}.yaml`, set)
    }
  }
