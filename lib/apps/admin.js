 ...
 * 同时能让类型系统看到字段名一定存在。
 */
const findSwitchTarget = (key) => {
    // Object.entries 会把键退化成 string，这里恢复其字面量类型
    for (const [file, values] of Object.entries(FileWitch)) {
 ...
 * 把配置节收敛成可按字符串键遍历的对象
 *
 * `Config.All()` 会读取 default_config 下的全部 yaml，各节必然存在；类型上它们是可选的，
 * 这里统一兜底为空对象，遍历与取值结果都与旧实现一致。
 */
const section = (value) => isRecord(value) ? value : {};
export class kkkAdmin extends plugin {
    ...
    async index_Settings(e) {
        ...
        const cookies = section(_cfg.cookies);
        const app = section(_cfg.app);
        const douyin = section(_cfg.douyin);
        const bilibili = section(_cfg.bilibili);
        const kuaishou = section(_cfg.kuaishou);
        const xiaohongshu = section(_cfg.xiaohongshu);
        const upload = section(_cfg.upload);
        const request = section(_cfg.request);
        const data = {
            // Cookies
            ...Object.fromEntries(Object.keys(cookies).map(k => [k, getStatus(cookies[k], true)])),
            // App
            ...Object.fromEntries(Object.keys(app).map(k => [k, getStatus(app[k])])),
            // Douyin
            ...Object.fromEntries(Object.keys(douyin).filter(k => k !== 'push').map(k => [k, getStatus(douyin[k])])),
            ...mapPushConfig('douyin', douyin.push),
            // Bilibili
            ...Object.fromEntries(Object.keys(bilibili).filter(k => k !== 'push' && k !== 'realCommentCount' && k !== 'videoQuality' && k !== 'maxAutoVideoSize').map(k => [k, getStatus(bilibili[k])])),
            bilibiliRealCommentCount: getStatus(bilibili.realCommentCount),
            videoQuality: getStatus(bilibili.videoQuality, false, false, true),
            maxAutoVideoSize: getStatus(bilibili.maxAutoVideoSize),
            ...mapPushConfig('bilibili', bilibili.push),
            // Kuaishou
            ...Object.fromEntries(Object.keys(kuaishou).map(k => [k, getStatus(kuaishou[k])])),
            // Xiaohongshu
            ...Object.fromEntries(Object.keys(xiaohongshu).map(k => [`xiaohongshu_${k}`, getStatus(xiaohongshu[k])])),
            // Upload
            ...Object.fromEntries(Object.keys(upload).map(k => [k, getStatus(upload[k])])),
            // Request
            timeout: getStatus(request.timeout),
            userAgent: getStatus(request['User-Agent']),
            ...mapProxyConfig(request.proxy)
        };
...
function mapProxyConfig(proxyConfig) {
    if (!isRecord(proxyConfig))
        return {};
    const auth = isRecord(proxyConfig.auth) ? proxyConfig.auth : undefined;
    return {
        // 代理开关
        ...
        // 代理用户名（敏感信息，需要脱敏）
        proxyusername: getStatus(auth?.username, false, true),
        // 代理密码（敏感信息，需要脱敏）
        proxypassword: getStatus(auth?.password, false, true)
    };
}
/**
 * 获取配置项的状态显示HTML
 * @param value - 配置项的值
 * @param isCookie - 是否为Cookie类型（会进行脱敏处理）
 * @param isSensitive - 是否为敏感信息（会进行脱敏处理）
 * @param isQuality - 是否为画质配置（会将数字转换为画质描述）
 * @returns 返回格式化的HTML状态字符串
 */
const getStatus = (value, isCookie = false, isSensitive = false, isQuality = false) => {
    ...
    // 如果值在映射表中，直接返回对应的HTML（旧实现直接以值为键，JS 会隐式转成字符串）
    const status = statusMap[String(value)];
    if (status)
        return status;
    // 处理数组类型：显示数组长度，空数组显示为关闭状态
    if (Array.isArray(value))
        return `<div class="cfg-status${value.length ? '' : ' status-off'}">已配置 ${value.length} 项</div>`;
    // 处理数字类型：如果是画质配置，将数字转换为画质描述（如 80 -> 高清 1080P）
    if (typeof value === 'number')
        return isQuality ? `<div class="cfg-status">${QualityMap[value] || value}</div>` : `<div class="cfg-status">${value}</div>`;
    // 处理对象类型：统一显示为“已配置”
    if (typeof value === 'object')
        return '<div class="cfg-status">已配置</div>';
    // 处理字符串类型
...
function checkNumberValue(value, limit) {
    ...
    if (min !== undefined && num < min)
        return min; // 小于最小值，返回最小值
    if (max !== undefined && num > max)
        return max; // 大于最大值，返回最大值
