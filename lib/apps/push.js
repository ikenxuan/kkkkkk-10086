...
import { bilibiliFetcher, buildAmagiRequestConfig, douyinFetcher } from '../module/utils/amagiClient.js';
import { DouYinpush, getDouyinID } from '../module/platform/douyin/index.js';
import { resolveDouyinUserByShortId } from '../module/platform/douyin/resolveUser.js';
import { Config, wrapWithErrorHandler } from '../module/utils/index.js';
import { isRecord } from '../module/utils/record.js';
import { isDefaultTool } from '../module/utils/app-config.js';
/**
 * 旧实现直接写 `data.data`，响应结构异常时会抛 TypeError 并冒泡到宿主的错误处理，
 * 这里沿用「取不到就抛」，不静默把 undefined 传给下游。
 */
const requireData = (response, label) => {
    if (!isRecord(response))
        throw new TypeError(`${label}返回结构异常`);
...
export class kkkPush extends plugin {
     ...
     */
    createPushTask(businessName, platform, fn) {
    ...
    async setdyPush(e) {
        ...
        await new DouYinpush(e).setting(requireData(data, '抖音搜索数据'));
        return true;
    }
    /**
     * @param e 包含消息信息的对象
    ...
    async setbiliPush(e) {
        ...
        if (match && match[1]) {
            ...
            const profile = requireData(data, 'B站用户主页数据');
    ...
    async changeBotID(e) {
        ...
        Config.update('pushlist', 'douyin', (current) => updateGroupIds(current));
        Config.update('pushlist', 'bilibili', (current) => updateGroupIds(current));
    ...
    async globalIgnore(e) {
        ...
        if (/(douyin|iesdouyin)\.com/.test(url))
            return await this.ignoreDouyinWork(e, url);
        if (/bilibili\.com/.test(url))
            return await this.ignoreBilibiliDynamic(e, url);
