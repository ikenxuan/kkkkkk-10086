...
let defaultAmagiRoutes;
const getDefaultAmagiRoutes = () => {
    defaultAmagiRoutes ||= require('@ikenxuan/amagi');
...
const createStreamVideo = (dependencies) => (req, res) => {
...
const createRenderVideoPage = (dependencies) => (req, res) => {
    const filename = getSafeFilename(req);
    const videoPath = dependencies.validateVideoRequest(filename);
    if (!videoPath) {
        sendNotFound(res, '视频文件不存在或文件名非法');
...
const createVideoEvents = (dependencies) => (req, res) => {
    const filename = getSafeFilename(req);
    ...
    const sendPayload = () => {
        ...
        if (removed && !current.removedAt)
            dependencies.markVideoPreviewRemoved(filename);
...
const mountAmagiRoutes = (app, routes) => {
...
export const createPluginServer = (overrides = {}) => {
    const { amagiRoutes = getDefaultAmagiRoutes(), ...videoOverrides } = overrides;
...
export const startPluginServer = (options = {}) => {
    if (serverInstance)
        return serverInstance;
    const { port = Number(Config.amagi?.APIServerPort || Config.app.APIServerPort || 4567), ...dependencies } = options;
