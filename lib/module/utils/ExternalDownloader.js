 ...
 * @param tool 工具名
 */
export const isToolAvailable = async (tool) => {
...
const buildCurlArgs = (options, resume) => {
    const args = [
        '--location',
    ...
    if (maxSpeed > 0)
        args.push('--limit-rate', String(maxSpeed));
    if (resume)
        args.push('--continue-at', '-');
    for (const header of flattenHeaders(options.headers))
...
const buildWgetArgs = (options, resume) => {
    const args = [
        '--quiet',
        '--max-redirect', '5',
        '--tries', '1',
...
const createError = (tool, exitCode, stderr) => {
    ...
    const error = new Error(`${tool} 下载失败（退出码 ${exitCode ?? '未知'}${status ? `, HTTP ${status}` : ''}）${detail ? `: ${detail}` : ''}`);
 ...
 */
export const isRetryableExternalFailure = (error) => {
    if (!isRecord(error))
        return false;
    if (error.slow === true)
        return true;
 ...
 * @param options 下载参数
 */
export const downloadWithTool = async (tool, options) => {
    ...
    return await new Promise((resolve, reject) => {
        ...
        if (options.onProgress || slowGuard) {
            poller = setInterval(() => {
                fs.promises.stat(options.filepath).then(stats => {
                    ...
                    if (!verdict.triggered)
                        return;
                    slowAbort = Object.assign(createSlowDownloadError(verdict.bytesPerSecond, floor), 
                    // 补上 ExternalDownloadError 的形状，好让上层那套「要不要换地址」的判定
                    // 对两个工具用同一个口径（`isRetryableExternalFailure` 读的是 `slow`）。
                    { exitCode: null, slow: true, stderr: '' });
...
/** 清掉工具探测缓存。测试用。 */
export const resetToolAvailability = () => {
    availability.clear();
};
/** 覆盖工具探测结果。测试用，免得单测依赖跑测机器上装没装 curl。 */
export const setToolAvailability = (tool, available) => {
