    ...
    async getReplyMessage(event) {
        ...
        if (Version.BotName === 'TRSS-Yunzai' && event.reply_id && event.getReply) {
            ...
            if (replyMsg) {
                ...
                const replyElements = sourceArray.flatMap(item => isRecord(item) && Array.isArray(item.message) ? item.message : []);
        ...
        if (['LagrangeCore', 'Lagrange.OneBot', 'OneBotv11'].includes(botAdapter)) {
            ...
            if (replyMsg && event.bot?.sendApi) {
                ...
                const replyText = await this.extractMessageText(Array.isArray(data) ? data : [], '引用消息');
                if (replyText)
                    event.msg = replyText;
            }
        }
