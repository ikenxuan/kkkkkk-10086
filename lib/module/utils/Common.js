...
const isValidChannel = (value) => Number.isFinite(value) && value >= 0 && value <= 255;
const summarizePixels = (pixelCount, readPixel) => {
    ...
    for (let pixelIndex = 0; pixelIndex < pixelCount; pixelIndex += pixelStep) {
        ...
        if (!Number.isFinite(alpha) || alpha < 20 || alpha > 255)
            continue;
        if (!isValidChannel(pixel.r) || !isValidChannel(pixel.g) || !isValidChannel(pixel.b))
            continue;
...
const ratioOrFallback = (value, fallback) => {
    if (value === undefined)
        return fallback;
    return Number.isFinite(value) && value >= 0 && value <= 1 ? value : null;
};
const decisionFromStats = (stats) => {
    const { averageLuma } = stats;
    if (!Number.isFinite(averageLuma) || averageLuma < 0 || averageLuma > 1)
        return null;
    ...
    if (darkRatio === null || brightRatio === null || vividRatio === null)
        return null;
...
export const decideCoverTheme = (input) => {
    ...
    if (input && typeof input === 'object' && 'averageLuma' in input) {
        return decisionFromStats(input);
    }
    return null;
};
const useDarkThemeByTime = (now = new Date()) => {
    const date = now instanceof Date ? now : new Date(now);
    const hour = date.getHours();
    return !(hour >= 6 && hour < 18);
};
/** 解析配置的主题模式；智能主题在封面不可用时回退到按时间自动切换。 */
export const resolveUseDarkTheme = (theme, cover, now = new Date()) => {
    if (theme === 1)
        return false;
    if (theme === 2)
        return true;
    if (theme === 3)
        return decideCoverTheme(cover)?.useDarkTheme ?? useDarkThemeByTime(now);
    return useDarkThemeByTime(now);
};
class Tools {
    tempDri;
    videoPreviews = new Map();
    constructor() {
    ...
    validateVideoRequest(filename) {
        ...
        if (!resolvedPath.startsWith(resolvedVideoDir + path.sep) && resolvedPath !== resolvedVideoDir)
            return null;
    ...
    async getReplyMessage(event) {
        ...
        if (Version.BotName === 'TRSS-Yunzai' && event.reply_id && event.getReply) {
            ...
            if (replyMsg) {
                ...
                const replyElements = sourceArray.flatMap(item => isRecord(item) && 'message' in item
        ...
        if (['LagrangeCore', 'Lagrange.OneBot', 'OneBotv11'].includes(botAdapter)) {
            const replyMsg = messages.find((msg) => typeof msg !== 'string' && msg.type === 'reply');
...
function normalizeMessageSegments(value) {
    ...
    return values.filter((item) => typeof item === 'string' || isRecord(item));
}
/**
 ...
 */
function qqbotReplySegments(elements) {
    const segments = [];
    for (const element of Array.isArray(elements) ? elements : []) {
        if (!isRecord(element))
            continue;
        const content = typeof element.content === 'string' ? element.content : '';
        if (content && !content.startsWith('<faceType='))
