 ...
 * 不该让一个脏数字把守卫变没。
 */
export const livePhotoBatchTimeoutMs = (imageCount) => {
    const count = Number.isFinite(imageCount) && imageCount > 0 ? Math.trunc(imageCount) : 0;
    return Math.min(MAX_MEDIA_TASK_TIMEOUT_MS, Math.max(MIN_MEDIA_TASK_TIMEOUT_MS, count * LIVE_PHOTO_BUDGET_PER_IMAGE_MS));
};
/**
 ...
 */
export const resolveMediaTaskTimeoutMs = (task, options = {}) => options.taskTimeoutMs?.[task] ?? options.timeoutMs;
export const runMediaTasks = async (tasks, options = {}) => {
    ...
    */
    ...
    if (tasks.image)
        entries.push(['image', tasks.image]);
    if (tasks.comment)
        entries.push(['comment', tasks.comment]);
    const settled = await Promise.allSettled(entries.map(([name, task]) => runWithRequestGuard(() => Promise.resolve().then(task), { timeoutMs: resolveMediaTaskTimeoutMs(name, options), maxRetries: 0 })));
    const result = {
        succeeded: [],
        failures: []
    };
