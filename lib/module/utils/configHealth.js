 ...
 */
export const recordConfigParseFailure = (file, error) => {
 ...
 * @param file 配置文件绝对路径
 */
export const recordConfigParseSuccess = (file) => {
    degradedFiles.delete(file);
};
/**
 * @returns 当前解析失败的配置文件，按文件名排序；全部正常时是空数组
 */
export const getDegradedConfigSnapshot = () => [...degradedFiles.values()].sort((a, b) => (a.file < b.file ? -1 : a.file > b.file ? 1 : 0));
/** 仅供测试：清空登记 */
export const resetConfigHealth = () => {
    degradedFiles.clear();
};
