...
import { getBilibiliData as fetchBilibiliData } from '../../module/platform/bilibili/api.js';
import { getDouyinData as fetchDouyinData } from '../../module/platform/douyin/api.js';
import { classifyCdnFailure, reportCdnFailure, resolveCdnCandidates } from './CdnRegistry.js';
...
let defaultAmagiModule;
const getDefaultAmagiModule = () => {
    defaultAmagiModule ||= require('@ikenxuan/amagi');
    return defaultAmagiModule;
};
const getAmagiDependencies = (overrides) => {
...
const getAmagiMethod = (target, property, getBilibiliData, getDouyinData) => {
    if (property === 'getBilibiliData')
        return getBilibiliData;
    if (property === 'getDouyinData')
        return getDouyinData;
 ...
 * 因此在代理里现场抓取真实调用栈作为兜底。
 */
const captureLiveStack = (name, message) => {
    const holder = {};
...
const collectApiDiagnostics = (platform, method, error, err) => {
    return [
...
/**
 * 把推送配置里的 `群号:机器人账号` 拆成目标列表。
 *
 * 两个平台各自的解析在 push.ts 里（douyin 丢掉缺 botId 的项、bilibili 留成空串），
 * 这里只为错误卡片服务，所以两者都保留、由调用方自己判空 —— 群号能显示就显示，
 * 哪怕这条配置漏了机器人账号。
 */
const parsePushTargets = (groupWithBot) => groupWithBot
    .map(entry => {
 ...
 * 定时推送时的适配器信息。
 *
 * 没有事件对象，所以按推送配置里的机器人账号主动去 `Bot[botId]` 取 —— 这个 bot 对象
 * 和事件里的 `event.bot` 是同一个（宿主 lib/bot.js 用 Proxy 暴露 `Bot`，事件上的 `bot`
 * 就是 `this.bots[self_id]`），因此 `getAdapterInfo` 不用改就能吃。
 *
 * 取不到（bot 离线、或配置里写了个已经不存在的账号）时返回 undefined，让卡片照旧
 * 不显示适配器区，而不是印一堆 unknown。
 */
const getPushAdapterInfo = (targets) => {
    for (const { botId } of targets) {
        ...
        if (!bot)
            continue;
        const info = getAdapterInfo({ bot });
        if (info)
            return info;
    }
    return undefined;
};
const buildApiErrorImage = async (platform, method, err, event, pushContext) => {
    const error = isRecord(err.error) ? err.error : isRecord(err.data) ? err.data : {};
    // 事件形状比 BaseEvent 宽；适配器与群/用户信息只做只读取值。
    const messageEvent = event;
    // 群/用户两行的场景判定与业务错误卡片共用 render.ts 那一套：
    // 这条路径正是定时推送用的接口错误卡片，压根没有触发者，原来的
    // `|| 'private'` / `|| 'unknown'` 会在图上印出「群: private」「用户: unknown」，
    // 看着像解析出错。两处口径必须一致，否则同一个占位符 bug 修一半留一半。
    //
    // 定时推送这条分支上事件为空，但目标群号在推送配置里是有的：走
    // `buildContextLogEntries(groupId, undefined)` 只补群号那行 —— 用户那行必须继续缺，
    // 定时推送确实没有触发者（这条不变量由 tests/unit/error-handler.test.ts 钉着）。
    const pushTargets = pushContext ? parsePushTargets(pushContext.groupWithBot) : [];
    const contextEntries = messageEvent
        ? buildEventContextLogEntries(messageEvent)
        : pushTargets.flatMap(target => buildContextLogEntries(target.groupId, undefined));
    ...
    try {
        ...
        return [
            ...
            `错误: ${name}: ${message}`,
            `平台: ${platform}`,
            // 文字回退跟卡片共用同一套场景判定：渲染挂了的时候这条是唯一的信息载体，
            // 不该比卡片多出「群: private」这种占位行。空串由下面的 filter(Boolean) 丢掉。
...
export class Base {
     ...
     * 当前正在处理的推送订阅。
     *
     * 定时推送没有事件对象，于是接口错误卡片既拿不到适配器信息、也拿不到目标群号
     * —— 卡片上只剩「哪个接口挂了」，看不出是哪个 bot 往哪个群推的时候挂的。
     * 这两样其实都在推送配置里（`group_id` 的元素形如 `群号:机器人账号`），
     * 但 `buildApiErrorImage` 是从 amagi 的 Proxy 里调的，那里只有 `event`，
     * 够不到订阅项。所以由 push 侧在进入每个订阅时写这里，Proxy 再读回去。
     *
     * 存原始的 `群号:机器人账号` 字符串数组而不是解析结果：两个平台的解析口径
     * 不一致（douyin 会丢掉缺 botId 的项，bilibili 保留成空串），在这里统一解析
     * 才不会把那点差异也一起带进错误卡片。
     */
    pushContext;
    headers;
     ...
     */
    amagi;
    constructor(e, overrides = {}) {
        this.e = e;
        /** @type {import('axios').AxiosRequestConfig['headers']} */
        this.headers = baseHeaders;
        const dependencies = getAmagiDependencies(overrides);
        const { default: Client, bilibiliErrorCodeMap, getBilibiliData, getDouyinData } = dependencies;
        let proxy = false;
        ...
        this.amagi = new Proxy(client, {
            get(target, prop) {
                const property = prop;
                const method = getAmagiMethod(target, property, getBilibiliData, getDouyinData);
                if (typeof method === 'function') {
                    return async (...args) => {
                        const rawResult = await Reflect.apply(method, target, args);
                        if (!rawResult) {
    ...
    mkbutton(btn) {
        ...
        if (['Miao-Yunzai', 'yunzai'].includes(Version.BotName)) {
            ...
            if (this.botadapter === 'QQBot' && this.e?.bot?.config?.markdown?.type !== 0) {
                const createButton = Reflect.get(Bot ?? {}, 'Button');
...
const sendMasterMessage = async (platform, img) => {
    ...
    if (Version.BotName === 'TRSS-Yunzai') {
        await Bot?.sendMasterMsg(segments);
        return;
    }
    ...
    for (const masterQQ of masterList) {
        await bot.pickFriend(masterQQ)?.sendMsg(segments);
    }
};
/**
 * 直接发送远端视频地址。
 ...
 */
const defaultUploadFileDependencies = {
    resolveBotAdapter: event => new Base(event).botadapter
};
const toMessageTarget = (value) => isRecord(value) ? value : undefined;
/**
 * 判断宿主的发送返回是不是「确实发出去了」。
 *
 ...
 */
export const needsGroupFileChannel = (botAdapter, sizeInMB) => sizeInMB > (LARGE_VIDEO_SEGMENT_ADAPTERS.includes(botAdapter) ? 102 : 75);
/**
 ...
 */
export const isRemoteVideoTooLargeForUrlSend = (botAdapter, sizeInMB) => {
 ...
 */
const failGroupFileChannel = (botAdapter, sizeInMB) => {
 ...
 * @returns {Promise<boolean>}
 */
export const uploadFile = async (e, file, videoUrl, options, dependencies = defaultUploadFileDependencies) => {
    ...
    try {
        ...
        if (useGroupFile) {
            ...
            if (botAdapter === 'ICQQ') {
                if (!target?.fs?.upload)
                    return failGroupFileChannel(botAdapter, newFileSize);
 ...
 */
const probeVideoSize = async (downloadOpt) => {
    const candidates = resolveCdnCandidates(downloadOpt.resource ?? '', [downloadOpt.video_url, ...(downloadOpt.candidates ?? [])]);
...
export const downloadFile = async (videoUrl, opt) => {
    ...
    const { filepath, totalBytes } = await new Networks({
        ...
        const color = Version.BotName === 'TRSS-Yunzai'
            ? (c) => logger.hex(c)
            : (c) => logger.chalk?.hex(c) ?? logger.hex(c);
        const barLen = 45;
