...
import { bilibiliFetcher, douyinFetcher } from '../../module/utils/amagiClient.js';
import { AmagiError, readAmagiFailureCode } from '../../module/platform/common/softError.js';
import { readRiskVoucher } from '../../module/platform/bilibili/riskVoucher.js';
import { classifyCdnFailure, reportCdnFailure, resolveCdnCandidates } from './Network/CdnRegistry.js';
 ...
 */
const wrapFetcherWithErrorCard = (fetcher, platform, self, fallbackEvent, bilibiliErrorCodeMap) => new Proxy(fetcher, {
    get: (target, prop) => {
        ...
        return async (...args) => {
            try {
                ...
                if (platform === 'bilibili') {
                    ...
                    if (code === undefined || !(code in bilibiliErrorCodeMap))
                        throw error;
                    // -352 且拿得到 voucher 时原样抛：riskControl 策略要按它去走验证码流程，
                    // 出卡片就把那条路堵死了。候选路径必须与策略侧同源，见 riskVoucher.ts
                    if (code === -352 && readRiskVoucher(error) && Object.keys(event ?? {}).length !== 0)
                        throw error;
                }
...
export class Base {
    ...
    constructor(e, overrides = {}) {
        ...
        const { default: Client, bilibiliErrorCodeMap, bilibiliFetcher: bilibili, douyinFetcher: douyin } = dependencies;
        let proxy = false;
        ...
        this.amagi = new Proxy(client, {
            get: (target, prop) => prop in wrapped ? wrapped[prop] : Reflect.get(target, prop)
        });
    }
    /**
