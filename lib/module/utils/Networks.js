...
import { classifyCdnFailure, reportCdnFailure, reportCdnSuccess, resolveCdnCandidates } from './CdnRegistry.js';
import { probeAndOrderCdnUrls } from './CdnProbe.js';
import Config from './Config.js';
import { clampConcurrency, getDownloadBudgetLimit, runWithDownloadSlot } from './DownloadBudget.js';
import { createSlowDownloadError, createSlowSpeedGuard, DEFAULT_SLOW_FLOOR_BYTES, DEFAULT_SUSTAIN_MS, isSlowDownloadAbort, SAMPLE_INTERVAL_MS } from './DownloadWatchdog.js';
import { getErrorMessage } from './error-message.js';
import { downloadWithTool, isRetryableExternalFailure, resolveDownloadTool } from './ExternalDownloader.js';
...
const MB = 1024 * 1024;
export const normalizeDownloadOptions = (options, uploadConfig) => {
 ...
 */
const normalizeSlowGuard = (uploadConfig) => {
 ...
 */
const nextCdnCandidate = (candidates, tried) => candidates.find(url => !tried.has(url));
/**
 * 剩下还没试过的候选地址，保持原有次序。
 *
 ...
 * @param tried 已经试过的地址
 */
const nextCdnCandidates = (candidates, tried) => candidates.filter(url => !tried.has(url));
/**
 ...
 */
const externalMinBytes = (uploadConfig) => {
...
export class Networks {
    ...
    async downloadStream(progressCallback, retryCount = 0, options = {}) {
        if (retryCount > 0)
            return await this.attemptDownloadStream(progressCallback, retryCount, options);
        return await runWithDownloadSlot(async () => await this.attemptDownloadStream(progressCallback, 0, options), { bucket: this.downloadBucket });
    }
    async attemptDownloadStream(progressCallback, retryCount, options) {
        ...
        if (!filepath)
            throw new TypeError('下载文件路径不能为空');
        // 候选地址：`this.url` 永远排在最前（它是调用方明确指定的那一条），后面跟上
        // 接口给的镜像和地址簿里记着的。`this.url` 是 readonly，所以换地址不能改实例，
        // 只能让每次尝试自己算出「这次用哪条」。
        const candidates = resolveCdnCandidates(options.resource ?? '', [this.url, ...(options.candidates ?? [])]);
        ...
        try {
            ...
            if (externalTool !== 'builtin' && rangeProbe?.total !== undefined && rangeProbe.total >= externalMinSize) {
                ...
                try {
                    ...
                    if (isRecord(error) && error.slow === true && error instanceof Error) {
                        slowAbort = Object.assign(error, { kkkSlowAbort: true });
                        throw slowAbort;
                    }
                    throw error;
                }
            ...
            if (failureKind !== null)
                reportCdnFailure(url, failureKind);
