...
import fs from 'fs';
import os from 'os';
const isCommandResult = (value) => typeof value === 'object' && value !== null &&
...
try {
    ...
    try {
        const icqq = await import(__rewriteRelativeImportExtension(`file://${process.cwd()}/plugins/ICQQ-Plugin/node_modules/icqq/lib/index.js`));
        core = icqq.core;
    }
    catch { }
}
...
async function UploadRecord(e, record_url, seconds = 0, transcoding = true, brief = '') {
    ...
    try {
        ...
        if (!bot?.sendUni) {
            ...
            if (!silkBuffer)
                return segment.record(record_url); // 转换失败，返回原始地址
            return segment.record(`base64://${silkBuffer.toString('base64')}`);
        }
        // 然后获取音频buffer和时长
        const result = await getPttBuffer(filePath, transcoding);
        if (!result.buffer)
            return segment.record(record_url); // 获取buffer失败，返回原始地址
        if (transcoding)
            await fs.promises.writeFile(filePath, result.buffer); // 将buffer写入临时文件
        ...
        if (rsp[2])
            errors.drop(rsp[2], rsp[3]);
        const ipValue = rsp[5];
        const portValue = rsp[6];
        const ip = (Array.isArray(ipValue) ? (ipValue[0] || ipValue) : ipValue);
        const port = (Array.isArray(portValue) ? (portValue[0] || portValue) : portValue);
        const params = {
            ver: 4679,
            ukey: rsp[7].toHex(),
...
async function prepareAudioFile(file) {
    ...
    try {
        ...
        if (Buffer.isBuffer(file) || (typeof file === 'string' && file.startsWith('base64://'))) {
            ...
            if (IS_WIN && localFile.startsWith('/'))
                localFile = localFile.slice(1);
