...
const sanitizeFilename = (filename) => sanitizeFilenameSegment(filename, 50, 'image');
...
const downloadImageBuffer = async (imageUrl, headers = {}) => {
    ...
    if (fs.existsSync(imageUrl))
        return await fs.promises.readFile(imageUrl);
    // 记入所属平台的连接预算：processImageUrls 是 Promise.all 全量展开的，
    // 一个 30 图的图集在这里会一次性开 30 条连接。额度由桶统一压住。
    const response = await runWithDownloadSlot(async () => await axios.get(imageUrl, {
...
export const processImageUrl = async (imageUrl, title, index, headers = {}) => {
    ...
    if (mode === 'url' || imageUrl.startsWith('base64://') || imageUrl.startsWith('file://'))
        return imageUrl;
    try {
