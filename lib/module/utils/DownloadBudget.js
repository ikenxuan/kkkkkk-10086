 ...
 * 不需要重启或重建桶。
 */
export const getDownloadBudgetLimit = () => {
    if (limitResolver !== undefined)
 ...
 */
export const withDownloadBucket = async (bucket, fn) => {
...
export const getCurrentDownloadBucket = () => storage.getStore()?.bucket;
const resolveBucketName = (explicit) => {
 ...
 * 取一格额度，没有额度时排队等。
 */
export const acquireDownloadSlot = async (options = {}) => {
 ...
 */
export const tryAcquireDownloadSlots = (count, options = {}) => {
...
/** 在一格额度内跑一段下载，结束后一定归还。 */
export const runWithDownloadSlot = async (fn, options = {}) => {
 ...
 * 不持有任何可以反过来影响队列的句柄。
 */
export const getDownloadBudgetSnapshot = () => {
    const limit = getDownloadBudgetLimit();
    return {
        limit,
 ...
 * 传 undefined 恢复成读配置。
 */
export const setDownloadBudgetLimitResolver = (resolver) => {
 ...
 */
export const resetDownloadBudget = () => {
    buckets.clear();
