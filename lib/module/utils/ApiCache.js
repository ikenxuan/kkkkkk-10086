 ...
 */
export const NEVER_CACHE_METHODS = {
    douyin: [],
 ...
 */
export const setApiCacheEnabledResolver = (resolver) => {
 ...
 */
export const isApiCacheEnabled = () => {
    if (enabledResolver)
 ...
 */
export const fingerprintCookie = (cookie) => {
...
const stableStringify = (value) => {
    ...
    const walk = (input) => {
        ...
        const object = input;
        if (seen.has(object))
            throw new TypeError('circular cache key component');
        seen.add(object);
...
export const resolveApiCacheTier = (platform, method) => API_CACHE_POLICY[platform][method];
/**
 ...
 */
export const buildApiCacheKey = (request) => {
 ...
 * 常态写入保持 O(1)。
 */
const writeEntry = (key, entry, now) => {
    entries.delete(key);
 ...
 * @returns 该缓存时返回 true
 */
const classifyFailure = (platform, failure) => {
    ...
    if (code !== undefined && RISK_CONTROL_CODES[platform].includes(code))
        return true;
    return false;
};
/**
 ...
 * 3. 其余（真成功）→ 按档位存。
 */
const classifyResolved = (platform, tier, value) => {
    if (isRecord(value) && value.success === false) {
        if (value.soft === true)
            return { ttlMs: API_CACHE_TTL_MS[tier], negative: false };
        if (classifyFailure(platform, value))
            return { ttlMs: API_CACHE_NEGATIVE_TTL_MS, negative: true };
        return undefined;
    }
    return { ttlMs: API_CACHE_TTL_MS[tier], negative: false };
};
 ...
 */
export const withApiCache = async (request, fetchValue) => {
    ...
    const shared = (async () => {
        try {
            ...
            if (classifyFailure(request.platform, error)) {
                const now = Date.now();
                writeEntry(key, { tier, expiresAt: now + API_CACHE_NEGATIVE_TTL_MS, negative: true, outcome: { kind: 'error', error } }, now);
            }
            throw error;
        }
    })();
 ...
 * 它们既不会被命中也不占实际价值。
 */
export const getApiCacheSnapshot = () => {
    const now = Date.now();
