 ...
 */
export const nextCdnCandidate = (candidates, tried) => candidates.find(url => !tried.has(url));
/**
 * 剩下还没试过的候选地址，保持原有次序。
 *
 ...
 * @param tried 已经试过的地址
 */
export const nextCdnCandidates = (candidates, tried) => candidates.filter(url => !tried.has(url));
/** 只取主机名给日志用。整条下载地址带着签名参数，打进日志既长又没有可读性。 */
export const readUrlHost = (url) => {
    try {
