...
import { createSlowDownloadError, createSlowSpeedGuard, isSlowDownloadAbort, SAMPLE_INTERVAL_MS } from './DownloadWatchdog.js';
...
export const attemptDownloadStream = async (context, progressCallback, retryCount, options) => {
    const { url: primaryUrl, headers, userAgent, proxy, axiosInstance, timeout: connectTimeout, maxRetries, downloadBucket, retry } = context;
    ...
    // 只能让每次尝试自己算出「这次用哪条」。
    const candidates = resolveCdnCandidates(options.resource ?? '', [primaryUrl, ...(options.candidates ?? [])]);
    ...
    const request = async (requestOptions) => {
        ...
        if (url.startsWith('https:'))
            config.httpsAgent = context.httpsAgent;
        else if (url.startsWith('http:'))
            config.httpAgent = context.httpAgent;
    ...
    try {
        ...
        const response = await axiosInstance(downloadConfig);
        clearTimeout(timeoutId);
