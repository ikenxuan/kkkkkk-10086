 ...
 * 文案也已经改成 Yunzai 版（「插件建议的 Yunzai 运行环境版本」），但在 `apps/help.ts` 渲染它之前一直没有调用点。
...
const readPackageField = (file, pick) => {
    try {
...
export const getRequiredYunzaiVersion = () => readPackageField(path.join(PluginPath, 'package.json'), pkg => {
    const engines = pkg.engines;
    if (typeof engines !== 'object' || engines === null)
        return undefined;
    const value = engines.yunzai;
 ...
 */
export const getCurrentYunzaiVersion = () => readPackageField(path.join(ClientPath, 'package.json'), pkg => typeof pkg.version === 'string' && pkg.version ? pkg.version : undefined);
/**
 * 宿主版本是否低于插件声明的最低要求。
 *
