...
import { createSlowDownloadError, createSlowSpeedGuard, SAMPLE_INTERVAL_MS } from './DownloadWatchdog.js';
...
export const downloadMultipart = async (options) => {
    ...
    const downloadedSoFar = () => progress.reduce((sum, value) => sum + value, 0);
    ...
    const downloadPart = async (range, index) => {
        for (let attempt = 0; attempt <= options.maxRetries; attempt++) {
            ...
            try {
                ...
                if (response.status !== 206 || !actual || actual.start !== range.start ||
                ...
                if (structural || !isRetryable(error) || attempt >= options.maxRetries)
                    throw error;
    ...
    try {
        ...
        if (stats.size !== options.total)
            throw new Error('多线程下载临时文件大小不匹配');
...
function createMultipartError(message, code) {
