 ...
 */
const buildLiveStreamHeaderArgs = (headers) => {
    if (!headers)
        return [];
    const args = [];
    const extraLines = [];
    ...
    if (extraLines.length > 0)
        args.push('-headers', `${extraLines.join('\r\n')}\r\n`);
    return args;
};
/**
 * `stat` 出文件字节数，拿不到就算 0。
 ...
 * 都是同一个结论 —— 没东西。
 */
const statRecordedBytes = async (filePath) => {
    try {
 ...
 */
export const recordLiveStream = async (options) => {
    const operation = '录制直播流';
