...
export async function ffmpeg(cmd, options) {
    // 移除命令前缀并添加完整路径
    cmd = cmd.replace(/^ffmpeg/, '').trim();
    cmd = `${getFFmpegPath()} ${cmd}`;
    if (options?.booleanResult)
        return await exec(cmd, { ...options, booleanResult: true });
    return await exec(cmd, options);
}
export async function ffprobe(cmd, options) {
    // 移除命令前缀并添加完整路径
...
export const loopVideoWithTransition = async (options) => {
    ...
    if (bgmPath) {
        ...
        const result = await ffmpeg(`-y ${inputArgs} ${bgmInputArgs} -filter_complex "${audioFilter}" ` +
