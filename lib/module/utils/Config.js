...
export class Cfg {
    ...
    get cookies() {
        ...
        const raw = this.getDefOrConfig('cookies');
        return {
     ...
     * @returns 是否真的写进了磁盘
     */
    update(name, key, mutate, type = 'config') {
        ...
        const document = reader.document.toJS();
        const current = (isRecord(document) ? document[key] : undefined);
    ...
    parseYamlRecordResult(file) {
        try {
            ...
            logger.error(`[Config] 解析配置文件失败，该文件的配置已全部退回默认值: ${file}\n` +
                '  插件不会自动修复它（覆盖会清掉你写的内容），改好或删掉该文件再重启才会恢复。' +
                '发送 #kkk版本 可以看到当前所有退回默认值的配置', error);
