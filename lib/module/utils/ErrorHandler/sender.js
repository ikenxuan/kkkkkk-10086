...
const replyToTrigger = async (ctx, message) => {
    if (!ctx.event?.reply)
        return false;
    try {
...
export const sendErrorToTrigger = async (ctx, message) => {
    if (!Config.app.errorLogSendTo?.includes('trigger'))
        return false;
 ...
 */
export const sendErrorToTriggerAsFallback = async (ctx, message) => await replyToTrigger(ctx, message);
