...
const looksLikeAddress = (value) => /^(wss?|https?):\/\//i.test(value);
...
const getLabels = (event) => {
    ...
    const safeValues = (record) => Object.entries(record)
 ...
 */
const asVersionText = (value) => {
    const text = asText(value);
    if (!text || /undefined|\bNaN\b/i.test(text))
        return '';
    if (/^v?\d/.test(text))
        return text;
...
const firstVersion = (...values) => values.map(asVersionText).find(Boolean) || '';
/**
 * 协议标准。取值同样对齐 Karin 的枚举（`onebot11` / `onebot12` / `satori` / `milky` / `qqbot`），
 * 一律小写：模板的 OneBot 角标判定写的是 `standard.includes('onebot')` ——大小写敏感，
 * 之前返回的 `OneBot` 永远命中不到，角标在 Yunzai 上从来没出现过。
 *
 * 出字由模板的 `formatStandard` 负责（`ktr/.../handlerError.tsx`）：品牌名有正规写法的查表，
 * 其余走 camelCase 兜底。别为了卡片好看在这里改大小写，会把角标判定一起弄坏。
 */
const getStandard = (labels, explicit) => {
    ...
    if (joined.includes('onebot12') || /co[mn]wechat/.test(joined))
        return 'onebot12';
 ...
 * 普通配置对象不会两者都有。
 */
const isWebSocketLike = (value) => typeof value.readyState === 'number' && typeof value.send === 'function';
/**
 * 通信方式。
 *
 ...
 *
 * Yunzai 侧改成认 socket 实例。三个探测位置对应三种挂法：
 * - `bot.ws`：宿主自己的约定（lib/bot.js 的 cleanupBotConnection 就是按它反查 bot 的）。
 *   TRSS 起 `WebSocketServer`、协议端反连进来的那批都在这里（OneBotv11 / ComWeChat /
 *   GSUIDCore / OPQBot），Milky 的 ws 模式是主动外连、socket 也放这儿。
 * - `adapter.ws`：宿主 Satori 适配器把 socket 挂在适配器实例上，bot 上没有。
 * - `bot.sdk.ws`：QQBot-Plugin 用的 @windtrace/qq-group-bot，socket 由 SDK 的
 *   SessionManager 赋在 `sdk` 上（lib/sessionManager.js 的 connect：`this.bot.ws = new WebSocket(...)`，
 *   那个 `this.bot` 是 sdk 自己）。少这一位 QQBot 就永远报 unknown。
 *
 * 方向判定用 `_isServer`：`ws` 包在两侧都会写这个布尔（lib/websocket.js 里客户端 false、
 * 服务端 true），比 `url` 可靠 —— `_url` 只在 initAsClient 里赋值（同文件 719 行），
 * 是个单向信号，取不到时分不清「服务端连接」还是「还没连上的客户端」。`url` 留作兜底。
 */
const getCommunication = (event, adapterRecord) => {
    ...
    if (socket) {
        if (typeof socket._isServer === 'boolean')
            return socket._isServer ? 'webSocketServer' : 'webSocketClient';
        return asText(socket.url) ? 'webSocketClient' : 'webSocketServer';
    }
...
export const getAdapterInfo = (event) => {
    ...
    const name = firstText(botVersion.app_name, adapterRecord.name, event.adapter_name, typeof adapter === 'string' ? adapter : '', adapterRecord.id, adapterRecord.platform, event.adapter_id, apk.display, botVersion.app_full_name, botVersion.name, 'unknown');
    ...
    botVersion.app_version, apk.version, botVersion.version, adapterRecord.version, botVersion.id, 'unknown');
    const platform = firstText(adapterRecord.platform, event.adapter_id, adapterRecord.id, typeof adapter === 'string' ? adapter : '', 'unknown');
    const protocol = firstText(adapterRecord.protocol, adapterRecord.protocol_name, PROTOCOL_RULES.find(rule => rule.pattern.test(labels.join(' ')))?.protocol, 'unknown');
    return {
        name,
        version,
        protocol,
