...
export const getConfigValue = (value, fallback) => value ?? fallback;
/** 视频解析总开关，`videoTool`（新）优先于 `videotool`（旧），只有显式 false 才算关 */
export const isVideoToolEnabled = (app) => getConfigValue(app?.videoTool, app?.videotool) !== false;
/**
 * 是否抢最高优先级（`-Infinity`）。
 *
 ...
 */
export const isDefaultTool = (app) => getConfigValue(app?.defaulttool, app?.videoTool) !== false;
