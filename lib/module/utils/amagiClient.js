...
const bridgedEmitters = new WeakSet();
/**
 ...
 */
export const ensureAmagiEventBridge = (module) => {
    const events = module.amagiEvents;
    // 这个 amagi 版本没有 emitter：取数照常，只是没有旁路日志
    if (!events || bridgedEmitters.has(events))
        return;
    ...
    events.on('api:error', data => {
        try {
            ...
            logger.warn(`[amagi] api:error ${String(data.platform)}/${String(data.methodType)} errorCode=${String(data.errorCode ?? '无')} message=${String(data.errorMessage ?? '')}`);
        }
        catch {
            // 留证不能替换业务错误
        }
 ...
 * @param method 方法名
 */
export const douyinGuest = (method) => {
    const fetcher = douyinFetcher;
