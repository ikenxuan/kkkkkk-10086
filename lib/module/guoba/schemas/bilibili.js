 ...
 */
import { divider, group, input, num, option, radio, select, sw } from '../../../module/guoba/helpers.js';
import { danmakuSchemas, displayContentOptions, permissionOptions, pushFilterSchemas, sendContentOptions } from '../../../module/guoba/shared.js';
...
/**
 * 直播画质编号，和上面那张稿件画质表**不是**同一套编号。
 *
 * 单独列一张是因为两边的数字含义完全不重叠（稿件的 80 是 1080P，直播这边 80 是流畅），
 * 复用 `bilibiliQualityOptions` 会让面板给出一批直播接口不认的档位。
 * 也刻意不带「自动根据大小选择」那一项：录制前拿不到时长与体积，没有可比的东西。
 */
const bilibiliLiveQnOptions = [
    option('杜比', 30000),
    option('4K', 20000),
    option('原画', 10000),
    option('蓝光', 400),
...
export const bilibili = [
    ...
    divider('B站直播录制'),
    num('bilibili.live.maxDuration', '单次录制时长上限', 1, 7200, '秒', '录制走 ffmpeg 的 -t，时间到了自己收口，所以填多少就等多少：填 600 就是一条命令占住十分钟，期间该会话的其它解析都在排队。另一头是体积，直播流不转码、落盘大小约等于「码率 × 时长」，原画档一分钟就有几十 MB，录完还要过「视频上传拦截阈值」那道闸，超了等于白录一场。默认 300 秒'),
    radio('bilibili.live.qn', '录制画质', bilibiliLiveQnOptions, '直播的画质编号和上面稿件那套「视频画质偏好」不是同一张表（稿件的 80 是 1080P，直播这边 80 是流畅），所以候选项也是单独一份。这里没有「自动根据大小选择」：录制前拿不到时长也拿不到体积，无从比较。填的档位不可用时（没有大会员、主播没开那么高、或该档没转码）由 B站 自己降级，卡片上显示的是降级后实际拿到的那一档，不是这里填的值'),
    divider('B站推送'),
