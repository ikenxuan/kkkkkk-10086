import fs from 'fs';
import { ffmpeg, ffprobe } from '../../../module/utils/FFmpeg.js';
const FONT_SIZE = {
    small: { size: 28, track: 38 },
    medium: { size: 36, track: 48 },
    large: { size: 46, track: 60 }
};
const isFontSizeName = (value) => value === 'small' || value === 'medium' || value === 'large';
...
Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text`;
    ...
    for (const item of items) {
        ...
        if (trackIndex < 0)
            trackIndex = Math.floor(item.time * 10) % trackCount;
