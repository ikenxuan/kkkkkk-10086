 ...
 * amagi 业务失败的本地等价错误类型。
 *
 * `utils/amagiClient.ts` 的 `wrapAmagiClient` 用递归 Proxy 把 `success !== true` 的信封
 * 统一抛成它，但同一个业务码仍然既可能是**抛出来的对象**、也可能是**返回值**
...
export class AmagiError extends Error {
    ...
    rawError;
    constructor(code, message, data, rawError) {
        super(message);
 ...
 */
export const readAmagiFailureCode = (value) => {
    if (!isRecord(value))
        return undefined;
 ...
 *
 * 失败本身是 `utils/amagiClient.ts` 的 Proxy **抛**出来的，{@link softFetch} 把白名单内
 * 那些转成返回值 —— 调用点接住后要靠这个函数认出来才能降级。
 *
 * @param value {@link softFetch} 软化后的返回值，或任意值
 * @param allowedCodes 该平台的白名单，取自 {@link SOFT_ERROR_CODES}
 * @returns 是白名单内的业务拒绝时为 true
 */
export const isSoftFailure = (value, allowedCodes) => {
...
const toSoftFailureResult = (code, error) => {
 ...
 *
 * @param fn 真正发请求的 thunk，通常是 `() => this.amagi.<平台>.fetchXxx(...)`
 * @param allowedCodes 该平台的白名单，取自 {@link SOFT_ERROR_CODES}
 */
export const softFetch = async (fn, allowedCodes) => {
    try {
        return await fn();
    }
    catch (error) {
