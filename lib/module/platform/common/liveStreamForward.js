 ...
 */
export const buildLiveStreamForward = async (e, headline, entries, title) => {
    if (entries.length === 0)
        return undefined;
    const nodes = [buildHeadlineNode(headline)];
 ...
 * @returns 一条节点的内容
 */
const buildHeadlineNode = (headline) => {
    const lines = [
