 ...
 */
const readChromeMajor = (userAgent) => {
 ...
 */
export const buildUserAgentHeader = (platform) => {
    ...
    if (configuredMajor < AMAGI_BUILTIN_CHROME_MAJOR[platform])
        return {};
    return { 'User-Agent': configured };
};
/**
 ...
 */
export const buildSharedUserAgentHeader = () => {
    const newest = Object.keys(AMAGI_BUILTIN_CHROME_MAJOR).reduce((max, key) => (AMAGI_BUILTIN_CHROME_MAJOR[key] > AMAGI_BUILTIN_CHROME_MAJOR[max]
        ? key
        : max), 'douyin');
