...
import { createEmojiNode, createLineBreakNode, createMentionNode, createRichTextDocument, createTextNode, createTopicNode, createWebLinkNode } from '../../utils/richtext/index.js';
const formatCount = (value) => value ?? 0;
export const getCommentLimit = () => Math.max(1, Number(Config.xiaohongshu.numcomment || 5));
/**
...
export const normalizeTagNames = (tags) => {
...
export const normalizeAtUsers = (atUsers) => {
    ...
    for (const tab of tabs) {
        for (const collection of tab?.collection || []) {
            for (const emoji of collection?.emoji || []) {
                if (emoji?.image_name)
                    result.push({ name: emoji.image_name, url: emoji.image });
            }
        }
    }
    return result;
 ...
 */
const buildXiaohongshuTokenRegExp = (mentions) => {
    ...
    parts.push('\\[[^[\\]]+\\]', // 表情字面量：[微笑R]
    '#[^#\\n]+#' // 话题：#话题名#
    );
    return new RegExp(parts.join('|'), 'g');
};
/** 纯文本 → 富文本节点：换行拆 lineBreak，裸 URL 拆 webLink，其余是 text */
const buildXiaohongshuPlainNodes = (text) => {
    const nodes = [];
 ...
 * @param options 构建选项
 */
export const buildXiaohongshuRichText = (text, emojiData = [], atUsers = [], options = {}) => {
    let raw = String(text || '');
    // 小红书的话题写法是 `#话题名[话题]#`，`[话题]` 只是个标记，不是表情
    if (options.stripTopicMarker)
        raw = raw.replace(/\[话题\]/g, '');
    raw = raw.trim();
    const toDocument = (nodes) => createRichTextDocument(nodes, { platform: 'xiaohongshu' });
    if (!raw)
        return toDocument([]);
    const emojiTable = new Map();
    for (const emoji of emojiData) {
        if (emoji?.name && emoji?.url)
            emojiTable.set(emoji.name, emoji.url);
    }
    ...
    for (const match of raw.matchAll(buildXiaohongshuTokenRegExp(mentions))) {
        ...
        const node = (() => {
            ...
            if (token.startsWith('#'))
                return createTopicNode(token.replace(/\[话题\]/g, ''));
            if (token.startsWith('[')) {
        ...
        if (!node)
            continue;
        if (match.index > lastIndex)
            nodes.push(...buildXiaohongshuPlainNodes(raw.slice(lastIndex, match.index)));
        nodes.push(node);
        lastIndex = match.index + token.length;
    }
    if (lastIndex < raw.length)
        nodes.push(...buildXiaohongshuPlainNodes(raw.slice(lastIndex)));
    return toDocument(nodes);
};
/** 评论图片 → 一个能用的 URL；拿不到给空串（调用点都按 falsy 处理） */
export const pickPictureUrl = (picture) => {
 ...
 */
const normalizeCommentPicture = (picture) => {
 ...
 */
export const buildNoteStatistics = (interactInfo) => {
    ...
    const flag = (key) => info[key] === true || info[key] === 'true';
    return {
        liked: flag('liked'),
 ...
 */
const sortAndLimitComments = (comments) => (Array.isArray(comments) ? comments : [])
 ...
 */
export const buildRenderComments = (comments, emojiData, noteId) => {
    const pickId = (item) => item.id || item.comment_id || `${item.user_info?.user_id || 'user'}-${item.create_time || Date.now()}`;
