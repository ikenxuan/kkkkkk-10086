...
const getCommentLimit = () => Math.max(1, Number(Config.xiaohongshu.numcomment || 5));
/**
 * 按配置数量分页获取小红书评论，避免只取首屏导致评论数量不足。
 * fetchComments 参数用于隔离 API wrapper，也便于在不触碰真实网络的情况下验证分页仲裁。
 */
export const fetchConfiguredNoteComments = async (data, fetchComments = async (options) => await getXiaohongshuData('评论数据', options)) => {
