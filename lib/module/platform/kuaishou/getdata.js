...
import { buildAmagiRequestConfig, kuaishouFetcher } from '../../../module/utils/amagiClient.js';
/**
 ...
 */
const KUAISHOU_GUEST_COOKIE = 'did=web_50424132d556424eb8fa8d27a612fda9; didv=1720860549000; kpf=PC_WEB; clientid=3; kpn=KUAISHOU_VISION';
/**
 ...
 */
const fetchKuaishou = async (method, options) => {
    // 按联合类型下标取方法会丢掉 amagi 的重载（只剩 2 参那条），所以收窄成本地调用签名
    const fetch = kuaishouFetcher[method];
    const result = unwrapAmagiResult(await fetch(options ?? {}, Config.cookies.kuaishou || KUAISHOU_GUEST_COOKIE, buildAmagiRequestConfig()));
    if (!result || result === '') {
 ...
 */
export const fetchKuaishouWorkComments = async (photoId) => await fetchKuaishou(KUAISHOU_METHODS.comments, { photoId });
/** 表情表那一跳。同 {@link fetchKuaishouWorkComments}，只有评论卡要它 */
export const fetchKuaishouEmojiList = async () => await fetchKuaishou(KUAISHOU_METHODS.emojiList);
