...
const collectMentionTokens = (text) => {
    const tokens = [];
 ...
 */
export const buildKuaishouRichText = (text, emojiData = []) => {
    const raw = String(text || '');
    const nodes = [];
    const toDocument = () => createRichTextDocument(nodes, { platform: 'kuaishou' });
    if (!raw)
        return toDocument();
 ...
 */
export default async function comments(data, emojidata, limit) {
