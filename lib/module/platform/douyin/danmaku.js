 ...
 */
const sanitizeArgument = (value) => value.replace(/[\r\n\0]/g, '');
...
const defaultFfprobeRunner = async (args, options) => {
    const { ffprobe } = await import('../../../module/utils/FFmpeg.js');
    return await ffprobe(args, options);
};
const defaultFfmpegRunner = async (args, options) => {
    const { ffmpeg } = await import('../../../module/utils/FFmpeg.js');
...
export async function burnDouyinDanmaku(videoPath, danmakuList, outputPath, options = {}) {
    ...
    try {
        ...
        // common/danmaku.ts:201 的写法就是这个样子，这里之前是唯一的例外。
