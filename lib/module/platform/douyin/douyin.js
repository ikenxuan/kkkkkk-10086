...
import { getDouyinMusicId, getDouyinMusicUrlCandidates, pickReachableMusicUrl } from './musicUrl.js';
import { mergeDouyinEmojiList, syncDouyinEmojiResourceOnce } from './emojiRes.js';
import { fetchDouyinMusicDetail, getDouyinMusicianName, resolveDouyinMusicInfo } from './musicInfo.js';
import { livePhotoBatchTimeoutMs, runMediaTasks } from '../../../module/utils/MediaTasks.js';
...
import { buildDouyinLiveHeadline, buildDouyinLivePayload, listDouyinLiveStreams } from './live.js';
...
import { buildLiveStreamForward } from '../../../module/platform/common/liveStreamForward.js';
import fs from 'fs';
import { at, firstUrl, isRecord } from '../../../module/utils/record.js';
/** 四条支线的失败日志文案，键就是 `MediaTaskName`，漏一个会在类型检查时报出来 */
const MEDIA_TASK_LABELS = {
    poster: '作品信息海报/回复',
    video: '视频下载、弹幕烧录与发送',
    image: '图集/合辑/实况图与背景音乐发送',
    comment: '评论图渲染与发送'
};
 ...
 */
export const pickDouyinPlayUrl = (playAddr) => playAddr?.url_list?.[0] || playAddr?.url_list?.[1] || playAddr?.url_list?.[2] || '';
/**
 ...
 */
const formatResolution = (playAddr) => {
...
export class DouYin extends Base {
    ...
    async RESOURCES(data) {
        try {
            ...
            switch (this.type) {
                case 'one_work': {
                    const VideoResponse = narrowApiResponse(await this.amagi.douyin.parseWork({
                    ...
                    const getEmojiList = () => {
                        emojiListPromise ??= (async () => {
                            try {
                                const emojiData = narrowApiResponse(await this.amagi.douyin.fetchEmojiList({ typeMode: 'strict' }, Config.cookies.douyin, buildAmagiRequestConfig()), 'Emoji数据');
                                const fromApi = Emoji(emojiData.data).filter((item) => typeof item.url === 'string' && item.url.length > 0);
                                // 接口那份是 web 的旧快照，缺的全是限时表情，并进本地资源包补上（见 emojiRes.ts）。
                                // 刻意不 await：同步扔到后台，这次用现成的缓存，下次解析才吃到新表 ——
                                // 评论渲染不该为一次网络往返排队
                                syncDouyinEmojiResourceOnce().catch(() => { });
                                return mergeDouyinEmojiList(fromApi);
                            }
                            catch (error) {
                     ...
                     */
                    let g_video_candidates = [];
                    /**
                     * 文件名与卡片标题用的作品名。
                     *
                     * 提到 fan-out 之前算一次：原来它由「图集支线」和「视频源挑选」各自赋值，
                     * 而 `sendVideoInfo` / `sendVideo` / `sendComment` 三条支线都读它 ——
                     * 图集变成并发支线之后那种「先写后读」的默契就不成立了。
                     * 长度上限与兜底名照原样按类型分（视频 80/『抖音视频』，图文 50/『抖音图集』），
                     * 文章保持 undefined，那条路上三个读取点都会退回 `aweme.desc`。
                     */
                    const g_title = isVideo
                        ? sanitizeFilenameSegment(VideoData.data.aweme_detail.preview_title, 80, '抖音视频')
                        : isArticle
                            ? undefined
                            : sanitizeFilenameSegment(VideoData.data.aweme_detail.preview_title, 50, '抖音图集');
                    /** 按画质配置选中、即将下载发送的那一路视频源，卡片上的清晰度从它派生 */
                    let selectedVideo;
                    /**
                     * 评论图页头那行「图片数量」。
                     *
                     * 直接取 `images` 的长度，不再靠图集支线里那些 `imagenum++` 累出来 ——
                     * 那些自增和这个长度逐条对应，但它们现在跑在另一条并发支线里，
                     * `sendComment` 读到的会是个还没数完的中间值。顺带修掉一个老问题：
                     * 关掉「图集」开关时那些自增压根不执行，评论卡上的图片数恒为 0。
                     */
                    const imagenum = (VideoData.data.aweme_detail.images ?? []).length;
                    /**
                     * 图文正文支线：图集 / 合辑 / 实况图，以及紧跟其后的背景音乐。
                     *
                     * 原来这一整段是 fan-out 之前的裸 await，而它同时是整条解析里最慢的一步
                     * （N 张图下载 + ffmpeg 逐张转实况图）—— 封面卡和评论卡全程被它堵着；
                     * 它一抛（某张图 403、ffmpeg 崩、makeForwardMsg 失败）那两张卡也一起没了。
                     *
                     * 背景音乐跟着搬进来，因为它的开关读的是 `this.hasProcessedLiveImage`，
                     * 而那个值由下面的图集/合辑分支写 —— 留在外面就成了跨支线读一个还没写完的值。
                     * 它本来也只对图文作品生效：`haspath` 要求 `is_mp4 === false`。
                     */
                    const sendWorkImages = !isVideo && !isArticle && (hasDouyinContent('图集') || hasDouyinContent('背景音乐'))
                        ? async () => {
                            // 这条支线只在「非视频且非文章」时构造，上面那个三元里正好是给 g_title
                            // 赋值的那一支，所以这里一定是字符串；`??` 只为让类型收窄。
                            const title = g_title ?? '抖音图集';
                            if (hasDouyinContent('图集')) {
                                switch (true) {
                                    // 图集
                                    case this.is_slides === false && VideoData.data.aweme_detail.images !== null: {
                                        ...
                                        if (hasLiveImage) {
                                            ...
                                            const livePhotoItems = images.map(imageItem => {
                                            ...
                                            for (const [index, imageItem] of images.entries()) {
                                                ...
                                                if (imageItem.url_list?.[0]) {
                                                    const imageUrl = await processImageUrl(imageItem.url_list[0], title, index, {
                                                        Referer: 'https://www.douyin.com/',
                                                        Cookie: Config.cookies.douyin || ''
                                                    });
                                        ...
                                        for (const [index, imageItem] of images.entries()) {
                                            ...
                                            if (Config.app.removeCache === false) {
                                                Common.mkdir(`${Common.tempDri.images}${title}`);
                                                const path = `${Common.tempDri.images}${title}/${index + 1}.png`;
                                                await new Networks({ url: image_url, type: 'arraybuffer' }).getData().then((data) => fs.promises.writeFile(path, data));
                                            }
                                        }
                                        if (imageres.length === 1) {
                                            await this.e.reply(segment.image(await processImageUrl(image_url, title, 0, {
                                                Referer: 'https://www.douyin.com/',
                                                Cookie: Config.cookies.douyin || ''
                                            })));
                                        }
                                            ...
                                            /*
                                              这个 await 不能省。`common.makeForwardMsg` 的签名是
                                              `unknown | Promise<unknown>`，宿主适配器返回 Promise 时原来这里
                                              把 Promise 本体塞进了 reply；同文件另外三处（实况图、合辑内容、
                                              评论图片收集）都是 await 过的，只有这条漏了。
                      
                                              对这次的支线拆分来说它还是正确性前提：不 await，makeForwardMsg
                                              的 reject 就成了 unhandledRejection，runMediaTasks 根本看不到，
                                              图集支线会被记成「成功」。
                                            */
                                            await this.e.reply(await common.makeForwardMsg(this.e, imageres, '解析完的图集图片'));
                                        }
                                        break;
                                    }
                                    // 合辑
                                    case VideoData.data.aweme_detail.is_slides === true && VideoData.data.aweme_detail.images !== null: {
                                        ...
                                        const mixLivePhotoItems = images1.map(item => {
                                        ...
                                        for (const [index, item] of images1.entries()) {
                                            ...
                                            if (liveFallbackUrl) {
                                                const imageUrl = await processImageUrl(liveFallbackUrl, title, index, {
                                                    Referer: 'https://www.douyin.com/',
                                                    Cookie: Config.cookies.douyin || ''
                                                });
                                        ...
                                        try {
                                            ...
                                            if (images.length === 0) {
                                                ...
                                                // 同上：漏 await 会让 reject 变成 unhandledRejection，绕过 runMediaTasks
                                                await this.e.reply(await common.makeForwardMsg(this.e, images, '合辑内容'));
                                            }
                                        }
                                        catch (error) {
                                            logger.error(error);
                            ...
                            if (!isArticle && VideoData.data.aweme_detail.music && hasDouyinContent('背景音乐') && !this.hasProcessedLiveImage) {
                                ...
                                // 逐个探活取第一条可达的镜像，全都不可达时退回旧行为让下游照原样报错
                                const musicCandidates = getDouyinMusicUrlCandidates(music);
                                const music_url = (await pickReachableMusicUrl(musicCandidates)) ?? getDouyinMusicUrl(music);
                                if (this.is_mp4 === false && Config.app.removeCache === false && music_url !== undefined) {
                                    try {
                                        const path = Common.tempDri.images + `${title}/BGM.mp3`;
                    ...
                    let cover = '';
                    /*
                      视频源挑选：纯计算，没有 IO，但三条支线都要读它的产物
                      （`g_video_url` / `g_video_candidates` / `cover` / `mp4size` / `FPS` /
                      `selectedVideo`），所以它必须留在 fan-out 之前。
          
                      外面这层 try/catch 是它留在串行前奏的代价的补偿：`douyinProcessVideos`
                      会抛（「接口没有返回任何视频源」），而抛在这里等于封面卡和评论卡也一起没了。
                      塌成「各变量保持默认值」之后，只有视频支线会因为拿不到地址而失败，
                      剩下两条照发。
                    */
                    try {
                        if (isVideo) {
              ...
              `);
                        ...
                        logger.error('[抖音] 视频源挑选失败，本次只发封面与评论', error);
                    }
                    ...
                    const sendVideoInfo = hasDouyinContent('视频', 'info')
                        ? async () => {
                            ...
                            if (Config.douyin.videoInfoMode === 'text') {
                                ...
                                let userProfile;
                                try {
                                    const userProfileData = narrowApiResponse(await this.amagi.douyin.fetchUserProfile({
                        ...
                        : undefined;
                    // `g_video_url` 空只有一种来路：上面那层 try/catch 兜住了视频源挑选的失败。
                    // 那种情况下这条支线没有地址可下，挂着不如不启动，让封面和评论安静发完。
                    const sendVideo = isVideo && Boolean(g_video_url) && hasDouyinContent('视频', 'video') && sendvideofile
                        ? async () => {
                            /*
                            ...
                            */
                            ...
                            if (this.forceBurnDanmaku || Config.douyin.burnDanmaku) {
                                try {
                                    const danmakuData = narrowApiResponse(await this.amagi.douyin.fetchDanmakuList({
                                        aweme_id: data.aweme_id ?? '',
                    ...
                    const sendComment = hasDouyinContent('评论图', 'comment')
                        ? async () => {
                            ...
                            */
                            const CommentsData = narrowApiResponse(await this.amagi.douyin.fetchWorkComments({
                                aweme_id: data.aweme_id ?? '',
                    ...
                    await runMediaTasks({
                        ...
                        video: sendVideo,
                        image: sendWorkImages,
                        comment: sendComment
                    }, {
                        /*
                          图文正文支线按图数放宽：整批实况图是「N 张图下载 + ffmpeg 逐张转码」，
                          工作量线性于图数，60s 的默认兜底对十几张的合辑形同虚设。数值口径与
                          小红书那条 image 支线共用 `livePhotoBatchTimeoutMs`。
            
                          视频支线**不放宽**：它现在正靠 60s 默认值兜底拦卡死的上传，
                          见 MediaTasks 里 VIDEO_DOWNLOAD_TIMEOUT_MS 的说明。
                        */
                        taskTimeoutMs: { image: livePhotoBatchTimeoutMs(imagenum) },
                        onTaskFailure: ({ task, error }) => {
                            const taskLabel = MEDIA_TASK_LABELS[task];
                ...
                case 'user_dynamic': {
                    const UserVideoListData = narrowApiResponse(await this.amagi.douyin.fetchUserVideoList({
                        sec_uid: data.sec_uid ?? '',
                        typeMode: 'strict'
                    }, Config.cookies.douyin, buildAmagiRequestConfig()), '用户主页视频列表数据');
                    const UserInfoData = narrowApiResponse(await this.amagi.douyin.fetchUserProfile({
                        sec_uid: data.sec_uid ?? '',
                        typeMode: 'strict'
                    }, Config.cookies.douyin, buildAmagiRequestConfig()), '用户主页数据');
                ...
                case 'music_work': {
                    ...
                    // 取数三段式，按数据质量与请求数排序，顺序别倒回去：
                    // music/detail 一次请求就带 mp3 与权威 user_count；游客接口免 cookie、免签名，
                    // 是前者被 Argus 拦下时的兜底；两条都挂才让 amagi 去撞，由既有 ErrorHandler 出卡。
                    // 各自缺什么见 musicInfo.ts 的模块注释
                    const resolved = (await fetchDouyinMusicDetail(musicId, {
                        fetchMusicInfo: async (id) => await douyinFetcher.fetchMusicInfo({ music_id: id, typeMode: 'strict' }, Config.cookies.douyin, buildAmagiRequestConfig())
                    })) ?? (await resolveDouyinMusicInfo(musicId, {
                        parseWork: async (awemeId) => await douyinFetcher.parseWork({ aweme_id: awemeId, typeMode: 'strict' }, Config.cookies.douyin, buildAmagiRequestConfig())
                    }));
                    let music = resolved?.music_info;
                    if (!music) {
                        const MusicData = narrowApiResponse(await this.amagi.douyin.fetchMusicInfo({
                            music_id: musicId,
                            typeMode: 'strict'
                        }, Config.cookies.douyin, buildAmagiRequestConfig()), '音乐数据');
                        music = MusicData.data.music_info;
                    }
                    if (!music)
                        throw new Error('音乐数据缺少 music_info');
                    const sec_uid = music.sec_uid;
                    const UserData = narrowApiResponse(await this.amagi.douyin.fetchUserProfile({ sec_uid, typeMode: 'strict' }, Config.cookies.douyin, buildAmagiRequestConfig()), '用户主页数据');
                    // 探活先于渲染：卡片上那个「音乐文件」按钮和后面真正上传的必须是同一条，
                    // 否则按钮可能指向一个已经打不开的镜像
                    const musicInfoUrl = (await pickReachableMusicUrl(getDouyinMusicUrlCandidates(music))) || getDouyinMusicUrl(music);
                    const musicianName = getDouyinMusicianName(music);
                    const img = await Render('douyin/musicinfo', {
                        image_url: firstUrl(music.cover_hd),
                        desc: music.title,
                        // 不能用 music_info.id，19 位 id 过 JSON 必丢精度；真值在 mid / id_str 上
                        music_id: getDouyinMusicId(music),
                        create_time: Time(0),
                        user_count: Common.count(music.user_count),
                        avater_url: firstUrl(music.avatar_large) || firstUrl(UserData.data.user.avatar_larger),
                        ...
                        // 模板拿 share_url 当二维码内容，空串会让二维码抛错、整张卡片渲不出来，
                        // 所以没拿到 mp3 时退回原声主页
                        share_url: musicInfoUrl || `https://www.douyin.com/music/${getDouyinMusicId(music) || musicId}`,
                    ...
                    await this.e.reply(this.mkMsg([
                        ...
                        `music_id: ${getDouyinMusicId(music)}\n`,
                        `BGM_Id: ${musicId}`
                    ], musicInfoUrl ? [{ text: '音乐文件', link: musicInfoUrl }] : []));
                    if (!musicInfoUrl) {
                        // 原声本体拿到了、mp3 没拿到。卡片已经发出去了，只补一句说明
                        await this.e.reply('该原声抖音未提供可下载的音频，已只发送原声信息', { reply: true });
                        return true;
                    }
                    await this.e.reply(await UploadRecord(getUploadRecordEvent(this.e), musicInfoUrl, 0, !Config.douyin.sendHDrecord));
                    return true;
                }
                case 'live_room_detail': {
                    ...
                    const room = await resolveDouyinLiveRoom({ sec_uid: data.sec_uid, room_id: data.room_id }, async (method, options) => method === 'fetchLiveRoomInfo'
                        ? await this.amagi.douyin.fetchLiveRoomInfo(options, Config.cookies.douyin, buildAmagiRequestConfig())
                        : await this.amagi.douyin.fetchUserProfile(options, Config.cookies.douyin, buildAmagiRequestConfig()));
                    if (!room.living) {
                    ...
                    // 地址就在上面那份已经拉到的 liveItem 里，这里不发任何新请求。
                    const streamForward = await buildLiveStreamForward(this.e, buildDouyinLiveHeadline({
                        anchor: room.anchor,
                        liveItem: room.liveItem,
                        webRid: room.webRid,
                        // 内部房间号，不是 URL 里的 web_rid。两个都可能缺，缺了就回落到 web_rid 那条链
                        roomId: room.anchor.room_id_str || room.liveItem.id_str || '',
                        secUid: room.anchor.sec_uid ?? ''
                    }), listDouyinLiveStreams(room.liveItem));
                    if (streamForward)
                        await this.e.reply(streamForward);
                    // 15 秒预览：排进独立队列就返回，不占这次解析的预算。录完由队列自己发。
                    // 链接照 share_url 的形状重建 —— 重启恢复时要拿它重新走一遍取流，
                    // 而原始消息里那条可能是短链、过期后解析不出房间号。
                    //
                    // 动态 import 不是风格选择：`livePreview` 静态依赖 db 层与 FFmpeg，
                    // 静态引进来会把这两条链拉进每一个 import 过 douyin.ts 的地方
                    // （十个单测就是这么在 `runtime/host/config` 上炸的）。
                    if (room.webRid) {
                        const { enqueueLivePreview } = await import('../../../module/platform/common/livePreview.js');
                        await enqueueLivePreview(this.e, 'douyin', room.webRid, `https://live.douyin.com/${room.webRid}`);
                    }
                    return true;
                }
                default:
