import { Base, Common, Render } from '../../../module/utils/index.js';
...
const getPartitionTitle = (liveData, liveItem) => liveData?.data?.data?.partition_road_map?.partition?.title ||
...
export class DouyinPushPreview extends Base {
    ...
    async renderList(pushType, url) {
        ...
        const listData = await getDouyinData(pushType === 'favorite' ? 'fetchUserFavoriteList' : 'fetchUserRecommendList', { sec_uid: iddata.sec_uid, number: 1, typeMode: 'strict' });
