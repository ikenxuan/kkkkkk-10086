        ...
        ${logger.cyan('访问地址')}：${logger.green(pushType === 'live' ? 'https://live.douyin.com/' + (pushItem.Detail_Data?.room_data?.owner?.web_rid || '') : 'https://www.douyin.com/video/' + actualAwemeId)}`);
                ...
                for (const target of pushItem.targets) {
                    try {
                      ...
                      `);
                                            const videoObj = douyinProcessVideos(workData.video.bit_rate, Config.upload.filelimit || 100);
                                            ...
                                            downloadUrl = await new Networks({
                                                url: (workData.video.bit_rate[0]?.play_addr.url_list?.[0] || workData.video.play_addr_h264.url_list[0] || workData.video.play_addr_h264.url_list[0]),
                                                headers: {
                                                    ...douyinBaseHeaders,
                                                    Cookie: ''
                                                }
        ...
        try {
            ...
            for (const item of filteredUserList) {
                try {
                    ...
                    logger.warn(`[抖音推送] 用户 ${item.remark || item.short_id || item.sec_uid || '未知'}本轮跳过：${error instanceof Error ? error.message : String(error)}`);
                    continue;
                }
            }
        }
