 ...
 */
export const getDouyinMusicId = (music) => {
    if (!isRecord(music))
        return '';
    const m = music;
 ...
 */
export const getDouyinMusicUrlCandidates = (music) => {
    if (!isRecord(music))
        return [];
    const m = music;
    const out = [];
    ...
    push(m.play_url?.uri);
    if (Array.isArray(m.play_url?.url_list))
        m.play_url.url_list.forEach(push);
    // 有些作品的原曲地址只在 extra 里
    try {
        const extra = JSON.parse(typeof m.extra === 'string' ? m.extra : '{}');
        if (isRecord(extra))
            push(extra.original_song_url);
    }
 ...
 */
export const pickReachableMusicUrl = async (candidates, options = {}) => {
