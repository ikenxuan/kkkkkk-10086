...
import { Common, Config, Networks, baseHeaders } from '../../../module/utils/index.js';
...
export async function douyinComments(data, emojidata) {
    ...
    for (const comment of data.data.comments) {
        ...
        const imageurl = comment.image_list &&
        ...
        const userintextlongid = comment.text_extra && comment.text_extra[0] && comment.text_extra[0].sec_uid
            ? comment.text_extra[0].sec_uid && comment.text_extra.map(extra => extra.sec_uid)
            : null;
        const search_text = comment.text_extra && comment.text_extra[0] && comment.text_extra[0].search_text
