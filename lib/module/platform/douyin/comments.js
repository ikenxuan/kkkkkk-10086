...
import { createEmojiNode, createLineBreakNode, createMentionNode, createRichTextDocument, createSearchKeywordNode, createTextNode } from '../../utils/richtext/index.js';
import { Config, Networks, baseHeaders } from '../../../module/utils/index.js';
import { firstUrl } from '../../../module/utils/record.js';
import { buildAmagiRequestConfig, douyinFetcher } from '../../../module/utils/amagiClient.js';
const extractMentionSecUids = (textExtra) => {
    if (!Array.isArray(textExtra) || textExtra.length === 0)
        return null;
    const secUids = textExtra
        .map(item => item?.sec_uid)
        .filter((secUid) => Boolean(secUid));
    return secUids.length > 0 ? secUids : null;
};
const extractSearchText = (textExtra) => {
    if (!Array.isArray(textExtra) || textExtra.length === 0)
        return null;
 ...
 */
const extractSearchTokens = (textExtra, text) => {
 ...
 */
const resolveMentionTokens = async (userIds) => {
 ...
 */
const buildDouyinRichText = async (text, emojiData, mentionUserIds, searchTokens = []) => {
 ...
 */
const processCommentImage = async (imageUrl) => {
    if (!imageUrl)
        return null;
    ...
    if (headers['content-type'] !== 'image/heic')
        return imageUrl;
...
const toSendableImage = (url) => url.startsWith('data:image/jpeg;base64,')
 ...
 */
const fetchReplyComments = async (awemeId, commentId, emojiData, imageUrls) => {
    if (!awemeId || !commentId)
        return [];
    let replyComment;
    try {
...
export async function douyinComments(data, emojidata = []) {
    ...
    for (const comment of data.data.comments) {
        ...
        if (processedImageUrl)
            imageUrls.push(toSendableImage(processedImageUrl));
        if (sticker)
