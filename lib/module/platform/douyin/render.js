 ...
 */
import { createHashtagNode, createLineBreakNode, createMentionNode, createRichTextDocument, createTextNode } from '../../utils/richtext/index.js';
import { format, fromUnixTime } from 'date-fns';
import { Common, Config, Render } from '../../../module/utils/index.js';
import { at } from '../../../module/utils/record.js';
import { buildAmagiRequestConfig, douyinFetcher } from '../../../module/utils/amagiClient.js';
import { buildDouyinResolutionInfo, isDouyinHdrStream } from './videoQuality.js';
import { getDouyinWorkCoverUrl, isDouyinArticle, isDouyinImage, isDouyinVideo, normalizeArticleImages, parseJsonSafely } from './workType.js';
/**
 * @param Desc - 作品原始描述文本
 * @returns 如果描述为空则返回默认提示，否则返回原文
 */
const desc = (Desc) => Desc === '' ? '该作品没有描述' : Desc;
/**
 ...
 */
const buildCooperationInfo = (Detail_Data) => {
    ...
    if (Detail_Data.author &&
        !rawCreators.some(c => (Detail_Data.author?.uid && c.uid && c.uid === Detail_Data.author.uid) ||
 ...
 */
const cdnAvatar = (uri) => 'https://p3-pc.douyinpic.com/aweme/1080x1080/' + uri;
/**
 * 获取作品作者头像，优先沿用用户主页的高清头像，解析侧缺少主页数据时回退到作品作者头像。
 */
const getUserAvatar = (user) => {
    if (user?.avatar_larger?.uri)
        return cdnAvatar(user.avatar_larger.uri);
 ...
 * @returns 图片列表数据
 */
const buildImageList = (images, fallbackCover) => {
 ...
 */
const splitTitleAndBody = (source) => {
 ...
 */
const buildDescRichText = async (text, textExtra, titleOffset = 0, mentionCache = new Map()) => {
    if (!text)
        return createRichTextDocument([], { platform: 'douyin' });
    const tokens = [
 ...
 */
const extractHashtagTokens = (body, textExtra, titleOffset = 0) => (textExtra ?? [])
    .filter((item) => item.type === 1 && !!item.hashtag_name && typeof item.start === 'number' && typeof item.end === 'number')
    .map(item => ({
 ...
 */
const resolveMentionTokens = async (text, textExtra, titleOffset, mentionCache) => {
    ...
    await Promise.all(uniqueSecUids.map(async (secUid) => {
        if (mentionCache.has(secUid))
            return;
        try {
            const userInfo = await douyinFetcher.fetchUserProfile({ sec_uid: secUid, typeMode: 'strict' }, Config.cookies.douyin, buildAmagiRequestConfig());
    ...
    return candidates.flatMap((item) => {
        ...
        if (!mentionText)
            return [];
        if (text.slice(item.start, item.end) !== mentionText)
            return [];
        return [{
                start: item.start,
                end: item.end,
...
const appendTextSegments = (text, target) => {
    if (!text)
        return;
 ...
 */
const extractIpLocation = (Detail_Data) => {
    let raw = Detail_Data.user_info?.data?.user?.ip_location;
    if (!raw)
        raw = Detail_Data.ip_location;
 ...
 */
const extractSuggestWord = (Detail_Data) => {
 ...
 */
const buildMusicInfo = (music) => {
    if (!music || typeof music !== 'object')
        return undefined;
    const extra = parseJsonSafely(music.extra);
 ...
 */
const getDefaultPushLabel = (aweme) => {
 ...
 */
export const renderWorkImage = async (options) => {
    const { Detail_Data, create_time, shareLink } = options;
    const aweme = Detail_Data;
    ...
    if (isDouyinArticle(aweme)) {
        const content = parseJsonSafely(Detail_Data.article_info?.article_content);
        const fe_data = parseJsonSafely(Detail_Data.article_info?.fe_data);
    ...
    if (isDouyinImage(aweme)) {
        ...
        const coverUrlList = at(Detail_Data.images, 0)?.url_list;
