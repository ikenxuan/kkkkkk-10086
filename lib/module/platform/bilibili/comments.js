import { createEmojiNode, createLineBreakNode, createMentionNode, createRichTextDocument, createTextNode } from '../../utils/richtext/index.js';
/**
 * 表情占位符 → 行内表情节点。
 *
 ...
 */
const splitByEmote = (text, emote) => {
    const names = Object.keys(emote ?? {}).filter(name => emote?.[name]?.url && text.includes(name));
    if (names.length === 0)
        return splitByLineBreak(text);
    // 长的键优先，避免 [笑] 把 [笑哭] 先切一半
    const pattern = new RegExp(`(${names.sort((a, b) => b.length - a.length).map(name => name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')})`);
...
const splitByMention = (nodes, members) => {
    ...
    return nodes.flatMap((node) => {
        if (node.type !== 'text')
            return [node];
        return node.text.split(pattern).flatMap((part) => {
            if (!part)
                return [];
...
/** 评论正文 → 富文本文档 */
const buildCommentMessage = (content) => createRichTextDocument(splitByMention(splitByEmote(content.message ?? '', content.emote), content.members), { platform: 'bilibili' });
/** 评论配图，模板按数量决定单图铺满还是两张缩略图 + 「+N」 */
const buildCommentPictures = (content) => (content.pictures ?? []).map(picture => picture?.img_src).filter((src) => !!src);
/** IP 属地，接口给的是「IP属地：广东」 */
const buildCommentLocation = (reply) => reply.reply_control?.location?.replace(/^IP属地[:：]?\s*/, '') || '';
/**
 * 大会员昵称颜色。原实现把整个昵称包成 `<span style=…>` 塞进 `uname`，
 * 而模板是 `uname` 出纯文本、颜色单独读 `unameColor`，
 * 于是那串 span 标签会连同尖括号一起印在卡片上。
 */
const buildUnameColor = (member) => member.vip?.vipStatus === 1 ? (member.vip.nickname_color || '#FB7299') : null;
/** 二级评论，字段是 SubCommentItem 的子集，没有置顶和回复数 */
const buildSubComment = (reply, upperMid) => ({
...
export function bilibiliComments(commentsData) {
    ...
    return (commentsData.data?.replies ?? [])
        ...
        const subReplies = (reply.replies ?? []).filter((item) => !!item);
        return {
            avatar: reply.member?.avatar ?? '',
            uname: reply.member?.uname ?? '',
            unameColor: buildUnameColor(reply.member),
