import { baseHeaders, Networks } from '../../../module/utils/index.js';
...
const getDefaultDependencies = () => {
    defaultDependencies ??= {
        convertAvToBv: async (options) => await bilibiliFetcher.convertAvToBv(options, Config.cookies.bilibili, buildAmagiRequestConfig())
    };
 ...
 */
const extractLiveRoomId = (link) => /live\.bilibili\.com(?:\/[^/?#]*)*?\/(\d+)(?:[/?#]|$)/.exec(link)?.[1];
/**
 * 解析B站分享链接，返回作品ID对象
 * @param url 分享链接
 * @param log 是否记录日志
 ...
 */
export const getBilibiliID = async (url, log = true, dependencies = getDefaultDependencies()) => {
    ...
    try {
        ...
        const urlPatterns = [
            ...
            [
                ...
                (url) => {
                    ...
                    return (/^https:\/\/(?:t|www)\.bilibili\.com\/(?:opus\/)?(\d+)/.test(url) ||
        ...
        for (const [name, test, extract] of urlPatterns) {
            if (test(longLink)) {
                ...
                if (log)
                    logger.info(`[B站链接] 类型: ${name}`, result);
                break;
            }
        }
    }
