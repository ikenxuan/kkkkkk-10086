...
import { Base, Render, Config, Networks, mergeFile, Common, baseHeaders, downloadFile, uploadFile, downloadVideo, needsGroupFileChannel, processImageUrl, sanitizeFilenameSegment } from '../../../module/utils/index.js';
        ...
        try {
            ...
            switch (this.Type) {
                ...
                case 'dynamic_info': {
                    ...
                    const commentLimit = bilibiliCommentLimit();
                    const rawCommentsData = dynamicInfo.data.data.item.type !== DynamicType.LIVE_RCMD &&
                        commentLimit > 0
                        // 同上：下面的 isSoftFailure 读返回值，裸 fetcher 会抛，所以这里也要包。
                        ? await softFetch(async () => await this.amagi.bilibili.fetchComments({
                    ...
                    */
                    ...
                    const userProfileData = await this.amagi.bilibili.fetchUserCard({ host_mid: Number(dynamicInfo.data.data.item.modules.module_author.mid), typeMode: 'strict' }, Config.cookies.bilibili, buildAmagiRequestConfig());
                    switch (dynamicInfo.data.data.item.type) {
                        ...
                        case DynamicType.AV: {
                            if (dynamicInfo.data.data.item.modules.module_dynamic.major.type === 'MAJOR_TYPE_ARCHIVE') {
                                ...
                                const INFODATA = await this.amagi.bilibili.fetchVideoInfo({ bvid, typeMode: 'strict' }, Config.cookies.bilibili, buildAmagiRequestConfig());
                                if (commentLimit > 0 && commentsData) {
                                    const commentsdata = bilibiliComments(commentsData.data);
                                    await this.e.reply(await Render('bilibili/comment', {
                                        Type: '动态',
    ...
    async downloadMergedStream({ videoUrl, audioUrl, danmakuList, tag, videoCandidates, audioCandidates, resourceKey }) {
        ...
        await mergeFile('二合一（视频 + 音频）', {
            ...
            callback: async (success, resultPath) => {
                 ...
                 */
                return await uploadFile(this.e, { filepath: filePath, totalBytes: fileSizeInMB, originTitle: this.downloadfilename }, '', { useGroupFile: needsGroupFileChannel(new Base(this.e).botadapter, fileSizeInMB) });
            }
        });
    }
    /**
