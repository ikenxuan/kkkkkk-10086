...
import { Base, Render, Config, Networks, mergeFile, Common, baseHeaders, downloadFile, uploadFile, downloadVideo, processImageUrl, sanitizeFilenameSegment } from '../../../module/utils/index.js';
import { buildAmagiRequestConfig, loadAmagiEnums } from '../../../module/utils/amagiClient.js';
import { burnDanmaku } from '../../../module/platform/common/danmaku.js';
import common from '../../../runtime/host/common.js';
import { bilibiliComments, checkCk, genParams } from './index.js';
import { buildBilibiliArticleCategories, buildBilibiliArticleRichText, buildBilibiliRichTextForwardNodes, formatBilibiliDynamicRichText, formatBilibiliVideoDescRichText, getHotBilibiliDanmaku, getUsernameMetadata } from './dynamicText.js';
import { extractBilibiliArticleImages } from './article.js';
import { createBilibiliRichTextForwardMessage } from './richtext-message.js';
import { buildLivePhotoMessagesBatch as buildCommonLivePhotoMessagesBatch, buildLivePhotoTipMessage } from '../../../module/platform/common/livePhoto.js';
import { bilibiliCommentLimit } from '../../../module/platform/common/commentLimit.js';
...
import { isSoftFailure, softFetch, SOFT_ERROR_CODES } from '../../../module/platform/common/softError.js';
import { fromSeconds, reportMedia } from '../../../module/utils/media-metrics.js';
import fs from 'fs';
...
const readStreamUrls = (stream) => [stream?.base_url, stream?.url, ...(stream?.backup_url ?? [])]
 ...
 */
export const pickBilibiliStreamUrl = (stream) => {
 ...
 */
export const collectBilibiliStreamUrls = (stream) => {
...
export const dedupeBilibiliVideoStreams = (streams) => {
    if (!streams)
        return [];
    const ids = new Set();
...
export const getBilibiliPayload = (response) => {
 ...
 * 没有任何消费者，照实填上，不额外造数据。
 */
const buildBangumiPayload = (result) => {
 ...
 */
export const getBilibiliStreamResolution = (stream) => {
...
export class Bilibili extends Base {
        ...
        try {
            ...
            switch (this.Type) {
                case 'one_video': {
                    ...
                    const infoData = await this.amagi.bilibili.fetchVideoInfo({ bvid: iddata.bvid, typeMode: 'strict' }, Config.cookies.bilibili, buildAmagiRequestConfig());
                    ...
                    const sendVideoInfo = hasBilibiliContent('简介', 'info') && (Config.bilibili?.displayContent || []).length > 0
                        ? async () => {
                            if (Config.bilibili.videoInfoMode === 'image') {
                                const userProfileData = await this.amagi.bilibili.fetchUserCard({ host_mid: Number(owner.mid), typeMode: 'strict' }, Config.cookies.bilibili, buildAmagiRequestConfig());
                                let hotDanmaku = [];
                    ...
                    const sendVideo = hasBilibiliContent('视频', 'video')
                        ? async () => {
                            ...
                            */
                            ...
                            await this.getvideo(Config.bilibili.videopriority === true
                    ...
                    const sendComment = hasBilibiliContent('评论图', 'comment')
                        ? async () => {
                            ...
                            const commentsData = await softFetch(async () => await this.amagi.bilibili.fetchComments({
                                number: commentLimit,
                                type: 1,
                                oid: infoData.data.data.aid.toString(),
                                typeMode: 'strict'
                            }, '', buildAmagiRequestConfig()), SOFT_ERROR_CODES.bilibili);
                ...
                case 'bangumi_video_info': {
                    const videoInfo = await this.amagi.bilibili.fetchBangumiInfo({ [iddata.isEpid ? 'ep_id' : 'season_id']: iddata.realid, typeMode: 'strict' }, Config.cookies.bilibili, buildAmagiRequestConfig());
                    this.islogin = (await checkCk()).Status === 'isLogin';
                    ...
                    await this.e.reply(this.mkMsg(this.botadapter === 'QQBot' ? `# ${videoInfo.data.result.season_title}\n---\n${msg}\r\r---\n请在60秒内输入 第?集 选择集数` : img, [
                    ...
                    if (iddata?.Episode) {
                        ...
                        this.downloadfilename = sanitizeFilenameSegment(videoInfo.data.result.episodes[Number(Episode) - 1]?.share_copy, 50, '');
                ...
                case 'dynamic_info': {
                    ...
                    const dynamicInfo = await this.amagi.bilibili.fetchDynamicDetail({ dynamic_id: iddata.dynamic_id, typeMode: 'strict' }, Config.cookies.bilibili, buildAmagiRequestConfig());
                    /*
                      评论图的唯一闸门，四种动态类型共用。
                    ...
                    */
                    ...
                    if (hasBilibiliContent('评论图', 'comment') &&
                        ...
                        try {
                            ...
                            rawCommentsData = await softFetch(async () => await this.amagi.bilibili.fetchComments({
                    ...
                    */
                    ...
                    if (commentsSoftClosed)
                        await this.e.reply('UP主已关闭评论区，无法获取评论');
                    const commentsData = commentsSoftClosed ? false : rawCommentsData;
                    /**
                     ...
                     */
                    const sendCommentCard = async (options) => {
                        if (!commentsData)
                            return;
                    ...
                    switch (dynamicInfo.data.data.item.type) {
                        ...
                        case DynamicType.DRAW: {
                            ...
                            const sendImages = async () => {
                                ...
                                const livePhotoItems = pics.map(item => (item?.url && item.live_url
                                ...
                                try {
                                    ...
                                    if (imgArray.length > 1)
                                        await this.e.reply(['QQBot', 'KOOKBot'].includes(this.botadapter) ? imgArray : await common.makeForwardMsg(this.e, imgArray, '动态图片'));
                                }
                                finally {
                            ...
                            const sendPoster = async () => {
                                ...
                                await this.e.reply(await Render('bilibili/dynamic/DYNAMIC_TYPE_DRAW', {
                                    ...
                                    text: replacetext(dynamicInfo.data.data.item.modules.module_dynamic.major?.opus?.summary?.text || '', dynamicInfo.data.data.item.modules.module_dynamic.major?.opus?.summary?.rich_text_nodes || []),
                        ...
                        case DynamicType.FORWARD: {
                            ...
                            */
                            const userProfileData = await getUserCard();
                            const text = replacetext(dynamicInfo.data.data.item.modules.module_dynamic.desc.text, dynamicInfo.data.data.item.modules.module_dynamic.desc.rich_text_nodes);
                                ...
                                await runDynamicTasks({
                                    poster: async () => {
                                        const [INFODATA, userProfileData] = await Promise.all([
                                            this.amagi.bilibili.fetchVideoInfo({ bvid, typeMode: 'strict' }, Config.cookies.bilibili, buildAmagiRequestConfig()),
                                            getUserCard()
                                        ]);
                                        const img = await Render('bilibili/dynamic/DYNAMIC_TYPE_AV', {
                                            ...
                                            dynamic_text: replacetext(dynamicInfo.data.data.item.modules.module_dynamic.desc?.text || '', dynamicInfo.data.data.item.modules.module_dynamic.desc?.rich_text_nodes || []),
                        ...
                        case DynamicType.LIVE_RCMD: {
                            ...
                            const userINFO = await this.amagi.bilibili.fetchUserCard({ host_mid: Number(dynamicInfo.data.data.item.modules.module_author.mid), typeMode: 'strict' }, Config.cookies.bilibili, buildAmagiRequestConfig());
                ...
                case 'live_room_detail': {
                    ...
                    const liveInfo = await this.amagi.bilibili.fetchLiveRoomInfo({ room_id: iddata.room_id, typeMode: 'strict' }, Config.cookies.bilibili, buildAmagiRequestConfig());
                    const roomInitInfo = await this.amagi.bilibili.fetchLiveRoomInitInfo({ room_id: iddata.room_id, typeMode: 'strict' }, Config.cookies.bilibili, buildAmagiRequestConfig());
                    const userProfileData = await this.amagi.bilibili.fetchUserCard({ host_mid: Number(roomInitInfo.data.data.uid), typeMode: 'strict' }, Config.cookies.bilibili, buildAmagiRequestConfig());
                    ...
                    const streamForward = await buildLiveStreamForward(this.e, {
                    ...
                    await enqueueLivePreview(this.e, 'bilibili', iddata.room_id, `https://live.bilibili.com/${iddata.room_id}`);
                    break;
                }
                default:
                    return true;
     ...
     */
    async downloadMergedStream({ videoUrl, audioUrl, danmakuList, tag, videoCandidates, audioCandidates, resourceKey }) {
        ...
        await mergeFile('二合一（视频 + 音频）', {
            ...
            callback: async (success, resultPath) => {
                ...
                /**
                 * 走不走群文件只听 `upload.usegroupfile` / `upload.groupfilevalue`。
                 *
                 * 这里原来按适配器算一个「消息段装不下」的上限并据此强制走群文件，现在不再区分
                 * 适配器 —— 多数适配器都能发到那个量级了。注意别再错用 `upload.filelimit`：
                 * 那是「多大就直接放弃上传」的闸门，默认已放开到 1536MB。
                 */
    ...
    async getvideo({ infoData, playUrlData, danmakuList = [] }) {
        ...
        if (this.islogin) {
            ...
            if (!durlUrl) {
                logger.mark(Config.cookies.bilibili
...
const additionalButtonText = (button) => {
    if (button?.jump_style?.text)
        return button.jump_style.text;
    return (button?.status === 2 ? button.check?.text : button?.uncheck?.text) ?? '';
};
/** 从「8054播放 · 15弹幕」里拆出两个数值。模板自己补「播放」「弹幕」后缀，所以只取数值段。 */
const parseUgcCounts = (descSecond) => ({
 ...
 */
export const buildBilibiliAdditionalCard = (additional) => {
...
const getContentRangeSize = (contentRange) => {
