 ...
 * 按路径读取 amagi 响应里的字段。
 *
 * 二维码两个接口的返回值不带具体结构，而旧实现直接写 `resp.data.data.url` 连续取值，
...
export const bilibiliLogin = async (e) => {
    /** 申请二维码 */
    const qrcodeurl = await bilibiliFetcher.requestLoginQrcode({ typeMode: 'strict' }, Config.cookies.bilibili, buildAmagiRequestConfig()); // 获取二维码URL
    ...
    const qrcodeMsg = await e.reply(await Render('bilibili/qrcodeImg', {
        share_url: shareUrl,
    ...
    while (true) {
        try {
            const qrcodeStatusData = await bilibiliFetcher.checkQrcodeStatus({ qrcode_key, typeMode: 'strict' }, Config.cookies.bilibili, buildAmagiRequestConfig());
