import { createAtNode, createEmojiNode, createLineBreakNode, createLotteryNode, createOpusLinkNode, createRichTextDocument, createTextNode, createTopicNode, createViewPictureNode, createVoteNode, createWebLinkNode } from '../../utils/richtext/index.js';
import { escapeHtml } from '../../../module/utils/html.js';
    ...
    for (const part of parts) {
        ...
        for (const match of part.matchAll(urlRegex)) {
            if (match.index > lastIndex)
                nodes.push(escapeHtml(part.slice(lastIndex, match.index)));
            const url = match[0];
            nodes.push(buildColoredText(url, useDarkTheme, url));
            lastIndex = match.index + url.length;
        }
        if (lastIndex < part.length)
            nodes.push(escapeHtml(part.slice(lastIndex)));
    }
    return nodes.join('');
};
...
export const formatBilibiliDynamicText = (text, richTextNodes = [], options = {}) => {
    ...
    if (!nodes.length)
        return parsePlainText(rawText, useDarkTheme);
    if (!rawText) {
 ...
 */
const buildPlainTextNodes = (text) => {
    const nodes = [];
    const parts = String(text || '').split(/(\r?\n)/);
    for (const part of parts) {
        if (part === '\n' || part === '\r\n') {
        ...
        for (const match of part.matchAll(urlRegex)) {
            if (match.index > lastIndex)
                nodes.push(createTextNode(part.slice(lastIndex, match.index)));
            const url = match[0];
            nodes.push(createWebLinkNode(url, url));
            lastIndex = match.index + url.length;
        }
        if (lastIndex < part.length)
            nodes.push(createTextNode(part.slice(lastIndex)));
    }
    return nodes;
};
/** B 站富文本节点 → 富文本节点，逐类对应 {@link buildNodeHtml} 的那个 switch */
const buildRichTextNodes = (node) => {
    const matchText = node?.orig_text || node?.text || '';
    if (!matchText)
        return [];
    switch (node?.type) {
        ...
        case 'RICH_TEXT_NODE_TYPE_EMOJI': {
            ...
            if (!url)
                return [createTextNode(matchText)];
 ...
 * @param richTextNodes B 站 rich_text_nodes
 */
export const formatBilibiliDynamicRichText = (text, richTextNodes = []) => {
    const rawText = normalizeInputText(text);
    const nodes = Array.isArray(richTextNodes) ? richTextNodes : [];
    const toDocument = (result) => createRichTextDocument(result, { platform: 'bilibili' });
    if (!nodes.length)
        return toDocument(buildPlainTextNodes(rawText));
    if (!rawText)
        return toDocument(nodes.flatMap(node => buildRichTextNodes(node)));
    const result = [];
    let currentPos = 0;
    for (const node of nodes) {
        const matchText = node?.orig_text || node?.text || '';
 ...
 * @param fallback desc_v2 缺失时使用的纯文本简介
 */
export const formatBilibiliVideoDescRichText = (descV2 = [], fallback = '') => {
    if (!Array.isArray(descV2) || descV2.length === 0) {
 ...
 * @param emojis 表情表
 */
export const applyBilibiliEmojiTable = (document, emojis) => {
    const table = new Map();
    for (const item of emojis ?? []) {
        if (item?.text && item?.url)
            table.set(item.text, item.url);
    }
    if (table.size === 0)
        return document;
    const nodes = [];
    for (const node of document.nodes) {
        ...
        for (const match of node.text.matchAll(/\[[^[\]]+\]/g)) {
            ...
            if (!url)
                continue;
            if (match.index > lastIndex)
                replaced.push(createTextNode(node.text.slice(lastIndex, match.index), node.style));
        ...
        if (lastIndex < node.text.length)
            replaced.push(createTextNode(node.text.slice(lastIndex), node.style));
        nodes.push(...replaced);
    }
 ...
 */
export const buildBilibiliArticleCategories = (categories) => {
    ...
    for (const item of categories) {
        if (typeof item === 'string') {
            if (item)
                result.push({ id: 0, name: item, parent_id: 0 });
            continue;
        }
        if (!item?.name)
            continue;
...
const createBilibiliArticleDocument = (nodes = []) => ({
    version: 1,
    platform: 'bilibili',
    nodes
});
const articleTextNode = (text, style) => Object.keys(style || {}).length > 0
    ? { type: 'text', text, style }
    : { type: 'text', text };
const decodeArticleHtmlEntities = (value) => {
    const named = {
...
const normalizeArticleImageUrl = (value) => {
    const url = decodeArticleHtmlEntities(String(value || '')).trim();
    if (!url || /^(?:javascript|vbscript):/i.test(url))
        return '';
    return url.startsWith('//') ? `https:${url}` : url;
};
const normalizeArticleLinkUrl = (value) => {
    const url = decodeArticleHtmlEntities(String(value || '')).trim();
    if (!url || /^(?:javascript|vbscript):/i.test(url))
        return '';
    return url.startsWith('//') ? `https:${url}` : url;
};
const parseArticleHtmlAttributes = (source) => {
    const attrs = {};
...
const parseArticleHtmlTree = (content) => {
...
const articleHtmlTextContent = (node, preserveWhitespace = false) => {
    if (node.kind === 'text')
        return decodeArticleHtmlEntities(node.value);
    if (node.tag === 'br')
        return '\n';
    if (ARTICLE_IGNORED_HTML_TAGS.has(node.tag))
        return '';
...
const articleStyleEquals = (left, right) => JSON.stringify(left || {}) === JSON.stringify(right || {});
const mergeArticleInlineStyle = (inherited, element) => {
    ...
    if (tag === 'font' && element.attrs.color)
        style.color = element.attrs.color.trim();
    ...
    for (const declaration of declarations) {
        ...
        if (property === 'font-style' && value.toLowerCase() === 'italic')
            style.italic = true;
        if (property === 'font-weight' && (value.toLowerCase() === 'bold' || Number.parseInt(value, 10) >= 600)) {
            style.bold = true;
        }
        if (property === 'text-decoration' && /line-through/i.test(value))
            style.strike = true;
    }
    return Object.keys(style).length > 0 ? style : undefined;
};
const isArticleInlineRichTextNode = (node) => {
    switch (node.type) {
        case 'text':
        case 'emoji':
        case 'mention':
...
const pushArticleInlineNode = (target, node) => {
...
const trimArticleInlineNodes = (source) => {
    ...
    if (firstText?.type === 'text')
        firstText.text = firstText.text.replace(/^\s+/, '');
...
const normalizeArticleHtmlInlineText = (value) => decodeArticleHtmlEntities(value)
    .replace(/\u00a0/g, ' ')
    .replace(/[\t\r\n\f ]+/g, ' ');
const articleImageFallbackText = (alt, caption) => {
...
const getArticleHtmlImageData = (element, caption) => {
    const src = normalizeArticleImageUrl(element.attrs.src || element.attrs['data-src'] ||
...
const findArticleHtmlElements = (node, predicate, result = []) => {
    if (node.kind === 'text')
        return result;
    if (predicate(node))
        result.push(node);
    for (const child of node.children)
        findArticleHtmlElements(child, predicate, result);
    return result;
};
const getArticleCodeLanguage = (element) => {
    ...
    const language = String(classMatch?.[1] || attrs['data-language'] || attrs['data-lang'] || attrs.lang || '').trim();
    return language || undefined;
};
const collectArticleInlineNodes = (node, inheritedStyle, target) => {
    if (node.kind === 'text') {
        const text = normalizeArticleHtmlInlineText(node.value);
        if (text)
            pushArticleInlineNode(target, articleTextNode(text, inheritedStyle));
        return;
    }
    if (ARTICLE_IGNORED_HTML_TAGS.has(node.tag))
        return;
    ...
    if (node.tag === 'img') {
        ...
        if (!image.src)
            pushArticleInlineNode(target, articleTextNode(articleImageFallbackText(image.alt), inheritedStyle));
        return;
    }
    const style = mergeArticleInlineStyle(inheritedStyle, node);
    for (const child of node.children)
        collectArticleInlineNodes(child, style, target);
};
const collectArticleMixedNodes = (node, inheritedStyle, target) => {
    if (node.kind === 'text') {
        const text = normalizeArticleHtmlInlineText(node.value);
        if (text)
            pushArticleInlineNode(target, articleTextNode(text, inheritedStyle));
    ...
    if (node.tag === 'img') {
        ...
        if (image.src) {
            ...
            pushArticleInlineNode(target, articleTextNode(articleImageFallbackText(image.alt, image.caption), inheritedStyle));
        }
        return;
    }
    if (node.tag === 'hr') {
    ...
    if (node.tag === 'pre') {
        ...
        if (content)
            target.push({ type: 'codeBlock', content, ...(language ? { language } : {}) });
        return;
    }
    const style = mergeArticleInlineStyle(inheritedStyle, node);
    for (const child of node.children)
        collectArticleMixedNodes(child, style, target);
};
const articleMixedNodesToBlocks = (mixedNodes) => {
    const result = [];
    let inlineBuffer = [];
    const flushInline = () => {
        ...
        inlineBuffer = [];
        if (nodes.length > 0)
            result.push({ type: 'paragraph', nodes });
    };
...
const isArticleHtmlBlockElement = (element) => ARTICLE_BLOCK_HTML_TAGS.has(element.tag) ||
    /^h[1-6]$/.test(element.tag) ||
    element.tag === 'img' || element.tag === 'pre' || element.tag === 'li';
const convertArticleHtmlFlow = (children) => {
    const result = [];
    let inlineBuffer = [];
    const flushInline = () => {
        const nodes = trimArticleInlineNodes(inlineBuffer);
...
const convertArticleHtmlList = (element) => {
    const listElements = element.children.filter((child) => child.kind === 'element' && child.tag === 'li');
    const items = [];
    for (const listElement of listElements) {
        const blocks = convertArticleHtmlFlow(listElement.children);
        let itemNodes = blocks;
        const firstBlock = blocks[0];
        if (blocks.length === 1 && firstBlock?.type === 'paragraph')
            itemNodes = firstBlock.nodes;
        if (itemNodes.length > 0)
            items.push({ type: 'listItem', nodes: itemNodes });
    }
    if (items.length === 0) {
        const inline = [];
        for (const child of element.children)
            collectArticleInlineNodes(child, undefined, inline);
        const nodes = trimArticleInlineNodes(inline);
        if (nodes.length > 0)
            items.push({ type: 'listItem', nodes });
    }
    return items.length > 0 ? [{ type: 'list', ordered: element.tag === 'ol', items }] : [];
};
const convertArticleHtmlFigure = (element) => {
...
const convertArticleHtmlElement = (element) => {
    const tag = element.tag;
    if (ARTICLE_IGNORED_HTML_TAGS.has(tag))
        return [];
    if (/^h[1-6]$/.test(tag)) {
        const inline = [];
        for (const child of element.children)
    ...
    if (tag === 'ul' || tag === 'ol')
        return convertArticleHtmlList(element);
    if (tag === 'pre') {
    ...
    if (tag === 'figure')
        return convertArticleHtmlFigure(element);
    if (tag === 'li') {
        const blocks = convertArticleHtmlFlow(element.children);
        const firstBlock = blocks[0];
...
const opusTextStyle = (word, useDarkTheme) => {
    ...
    if (source.strike)
        style.strike = true;
    if (typeof source.link === 'string' && source.link)
        style.link = normalizeArticleLinkUrl(source.link);
    const color = useDarkTheme ? word.dark_color : word.color;
    if (!style.color && typeof color === 'string' && color)
        style.color = color;
    if (typeof source.color === 'string' && source.color)
        style.color = source.color;
    return Object.keys(style).length > 0 ? style : undefined;
};
const opusWordLinesToNodes = (word, useDarkTheme) => {
    ...
    for (const part of words.split(/(\r?\n)/)) {
        if (part === '\n' || part === '\r\n')
            ...
            pushArticleInlineNode(result, articleTextNode(part, style));
    }
    return result;
};
const isOrderedOpusList = (value) => {
    if (typeof value === 'number')
        return value === 1;
    const normalized = String(value || '').toLowerCase();
    return ['ordered', 'order', 'ol', 'number', 'numbered', 'decimal'].includes(normalized);
};
const opusHeadingLevel = (paragraphs, headingType) => {
...
const opusParagraphFallbackText = (paragraph) => {
    const card = paragraph.link_card?.card;
    if (paragraph.para_type === 7)
        return card?.show_text || paragraph.link_card?.default_text || '链接卡片';
    return '';
};
/** 将 B 站 opus 段落结构转换为宿主无关富文本文档。 */
export const parseOpusToRichText = (opus, useDarkTheme = false) => {
    ...
    for (const paragraph of paragraphs) {
        ...
        if (paraType === 7) {
            ...
            if (url) {
                ...
                if (fallback)
                    result.push({ type: 'paragraph', nodes: [articleTextNode(fallback)] });
            }
            continue;
        }
        if (paraType === 8) {
 ...
 */
export const buildBilibiliArticleRichText = (opus, content, useDarkTheme = false) => {
    if (Array.isArray(opus?.content?.paragraphs))
...
const normalizeBilibiliForwardText = (text) => text
    .replace(/\r\n/g, '\n')
    .replace(/[ \t]+\n/g, '\n')
    .replace(/\n{3,}/g, '\n\n')
    .trim();
/** 按消息文本上限拆分，同时尽量在段落/换行边界切分并保留所有字符。 */
export const splitBilibiliRichText = (text, limit = MAX_BILIBILI_FORWARD_TEXT_LENGTH) => {
    if (!text)
        return [];
    ...
    while (rest.length > safeLimit) {
        ...
        if (blankLine >= 0 && blankLine + 2 <= safeLimit)
            splitAt = blankLine + 2;
        if (splitAt < Math.floor(safeLimit / 2)) {
            const newline = rest.lastIndexOf('\n', safeLimit - 1);
            if (newline >= Math.floor(safeLimit / 2))
                splitAt = newline + 1;
        }
        if (splitAt < Math.floor(safeLimit / 2)) {
            const whitespace = Math.max(rest.lastIndexOf(' ', safeLimit - 1), rest.lastIndexOf('\t', safeLimit - 1));
            if (whitespace >= Math.floor(safeLimit / 2))
                splitAt = whitespace + 1;
        }
        if (splitAt <= 0 || splitAt > safeLimit)
            splitAt = safeLimit;
...
const formatBilibiliForwardLink = (text, url) => {
    if (!url || url === text)
        return text;
    return `${text} (${url})`;
};
const bilibiliInlineNodeToForwardText = (node) => {
    switch (node.type) {
        case 'text':
 ...
 */
export const buildBilibiliRichTextForwardNodes = async (document, options = {}) => {
    const result = [];
    let textBuffer = '';
    let imageIndex = 0;
    const maxTextLength = Number.isFinite(options.maxTextLength) && (options.maxTextLength || 0) > 0
        ? Math.max(1, Math.floor(options.maxTextLength))
        : MAX_BILIBILI_FORWARD_TEXT_LENGTH;
    ...
    const flushText = () => {
        ...
        textBuffer = '';
        for (const chunk of splitBilibiliRichText(text, maxTextLength))
    ...
    const appendImage = async (node) => {
        flushText();
        const index = imageIndex++;
        let resolved = node.src || null;
        let failed = !resolved;
    ...
    const headers = [];
    if (options.title)
        headers.push(`标题：${options.title}`);
    if (options.summary)
        headers.push(`简介：${options.summary}`);
    if (options.shareUrl)
        headers.push(`链接：${options.shareUrl}`);
    if (headers.length > 0) {
