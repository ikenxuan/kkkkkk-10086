...
export const parseBilibiliLiveStartedAt = (liveTime) => {
    const normalizedLiveTime = liveTime.trim();
    if (invalidLiveTimes.has(normalizedLiveTime))
        return null;
...
export const buildBilibiliLiveSessionId = (hostMid, roomId, liveTime) => {
    const normalizedLiveTime = liveTime.trim();
    if (!parseBilibiliLiveStartedAt(normalizedLiveTime))
        return null;
