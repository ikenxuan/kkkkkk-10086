...
import { withDownloadBucket } from '../../../module/utils/DownloadBudget.js';
    ...
    async checkremark() {
        ...
        for (const item of pushList) {
            if (item.remark !== undefined && item.remark !== '')
                continue;
            // 从外部数据源获取用户备注信息
            const resp = asAmagiResponse(await this.amagi.getBilibiliData('用户主页数据', { host_mid: item.host_mid, typeMode: 'strict' }));
