import { Base, baseHeaders, Common, Config, downloadFile, mergeFile, Render, uploadFile, Version, processImageUrl, sanitizeFilenameSegment } from '../../../module/utils/index.js';
...
import { applyBilibiliEmojiTable, buildBilibiliArticleCategories, buildBilibiliArticleRichText, buildBilibiliRichTextForwardNodes, formatBilibiliVideoDescRichText, getUsernameMetadata } from './dynamicText.js';
...
import { loadAmagiEnums } from '../../../module/utils/amagiClient.js';
 ...
 */
const isParseDynamicTypeAllowed = (dynamicType) => {
    const configured = Config.bilibili?.push?.parseDynamicTypes;
    if (!Array.isArray(configured))
        return true;
    return configured.includes(dynamicType);
};
...
export class Bilibilipush extends Base {
    ...
    ensureConfigFields(pushList) {
        ...
        const needsFix = pushList.some(item => item.switch === undefined ||
            !Array.isArray(item.pushTypes) ||
            item.pushTypes.join(',') !== normalizeBilibiliPushTypes(item.pushTypes).join(','));
        if (!needsFix)
            return;
        Config.update('pushlist', 'bilibili', (current) => {
    ...
    async getdata(data) {
        ...
        ${logger.cyan('访问地址')}：${logger.green('https://t.bilibili.com/' + dynamicId)}`);
                ...
                if (!skip) {
                    const userINFO = asAmagiResponse(await this.amagi.bilibili.fetchUserCard({ host_mid: dynamicItem.host_mid, typeMode: 'strict' }, Config.cookies.bilibili, buildAmagiRequestConfig()));
                    const emojiResponse = asAmagiResponse(await this.amagi.bilibili.fetchEmojiList({}, Config.cookies.bilibili, buildAmagiRequestConfig()));
                    ...
                    switch (dynamicItem.dynamic_type) {
                        ...
                        case DynamicType.DRAW: {
                            ...
                            img = await Render('bilibili/dynamic/DYNAMIC_TYPE_DRAW', {
                                ...
                                text: replacetext(dynamicItem.Dynamic_Data.modules.module_dynamic.major?.opus?.summary?.text || '', dynamicItem.Dynamic_Data.modules.module_dynamic.major?.opus?.summary?.rich_text_nodes || []),
                        ...
                        case DynamicType.WORD: {
                            ...
                            // 剩下的 `[表情名]` 字面量再用表情表补一遍
                            const text = applyBilibiliEmojiTable(replacetext(dynamicItem.Dynamic_Data.modules.module_dynamic.desc?.text || '', dynamicItem.Dynamic_Data.modules.module_dynamic.desc?.rich_text_nodes || []), emojiDATA);
                        ...
                        case DynamicType.AV: {
                            if (dynamicItem.Dynamic_Data.modules.module_dynamic.major?.type === 'MAJOR_TYPE_ARCHIVE') {
                                ...
                                const INFODATA = asAmagiResponse(await bilibiliFetcher.fetchVideoInfo({ bvid, typeMode: 'strict' }, '', buildAmagiRequestConfig()));
                                ...
                                img = await Render('bilibili/dynamic/DYNAMIC_TYPE_AV', {
                                    ...
                                    dynamic_text: replacetext(dynamicItem.Dynamic_Data.modules.module_dynamic.desc?.text || '', dynamicItem.Dynamic_Data.modules.module_dynamic.desc?.rich_text_nodes || []),
                                ...
                                switch (dynamicItem.dynamic_type) {
                                    case 'DYNAMIC_TYPE_AV': {
                                        if (send_video) {
                                            ...
                                            const simplify = (playUrlDash.video || []).filter((item, index, self) => {
                                            ...
                                            const infoData = asAmagiResponse(await this.amagi.bilibili.fetchVideoInfo({ bvid: dycrad.bvid, typeMode: 'strict' }, Config.cookies.bilibili, buildAmagiRequestConfig()));
                                                    ...
                                                    callback: async (success, resultPath) => {
                                                        if (success) {
                                                            ...
                                                            if (fileSizeInMB > (Config.upload?.groupfilevalue || 100)) {
                                                                return await uploadFile(this.e ?? {}, { filepath: filePath, totalBytes: fileSizeInMB, originTitle: sanitizeFilenameSegment(infoData.data.data.desc, 50, 'B站视频') }, '', { useGroupFile: true, active: true, activeOption: { group_id: groupId, uin: botId } });
                                                            }
                                                            else {
                                                                /** 因为本地合成，没有视频直链 */
                                    ...
                                    case 'DYNAMIC_TYPE_DRAW': {
                                        ...
                                        try {
                                            ...
                                            const livePhotoItems = images.map(img2 => {
     ...
     */
    async collectDirectLivePushItem(item, handledUids, willBePushList) {
        let liveStatus;
        try {
            const response = asAmagiResponse(await this.amagi.bilibili.fetchUserLiveStatus({
        ...
        try {
            const response = asAmagiResponse(await this.amagi.bilibili.fetchLiveRoomInfo({
        ...
        try {
            ...
            for (const item of filteredUserList) {
                try {
                    ...
                    this.pushContext = { groupWithBot: item.group_id };
                    const dynamic_list = asAmagiResponse(await this.amagi.bilibili.fetchUserDynamicList({ host_mid: item.host_mid, typeMode: 'strict' }, Config.cookies.bilibili, buildAmagiRequestConfig()));
                ...
                `);
                    ...
                    logger.warn(`[Bilibili 推送] UP主 ${item.remark}（${item.host_mid}）本轮跳过：${getErrorMessage(error)}`);
                    continue;
                }
                finally {
    ...
    async checkremark() {
        ...
        for (const item of pushList) {
            if (item.remark !== undefined && item.remark !== '')
                continue;
            const resp = asAmagiResponse(await this.amagi.bilibili.fetchUserCard({ host_mid: item.host_mid, typeMode: 'strict' }, Config.cookies.bilibili, buildAmagiRequestConfig()));
    ...
    async renderPushList() {
        ...
        for (const subscription of subscriptions) {
            ...
            const userInfo = asAmagiResponse(await this.amagi.bilibili.fetchUserCard({ host_mid, typeMode: 'strict' }, Config.cookies.bilibili, buildAmagiRequestConfig()));
