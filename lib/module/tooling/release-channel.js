...
const stripRemote = (ref) => ref.replace(/^[^/]+\//, '');
/**
 ...
 */
export const resolveChannel = (branch) => {
    if (!branch)
        return 'Dev';
...
export const getReleaseChannel = () => {
    ...
    if (state.source === 'git' && (state.dirty !== false || state.ahead !== 0))
        return 'Dev';
    return resolveChannel(state.branch);
};
/** 仅供测试：清掉状态缓存。 */
export const resetInstallStateCache = () => {
