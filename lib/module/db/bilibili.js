...
export class BilibiliDBBase {
    ...
    async getOrCreateBilibiliUser(host_mid, remark = '') {
        ...
        if (!user) {
            const now = new Date().toISOString();
            await this.runQuery('INSERT INTO BilibiliUsers (host_mid, remark, filterMode, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?)', [host_mid, remark, 'blacklist', now, now]);
            user = {
                host_mid,
                remark,
    ...
    async subscribeBilibiliUser(groupId, botId, host_mid, remark = '') {
        ...
        if (!subscription) {
            const now = new Date().toISOString();
            await this.runQuery('INSERT INTO GroupUserSubscriptions (groupId, host_mid, createdAt, updatedAt) VALUES (?, ?, ?, ?)', [groupId, host_mid, now, now]);
    ...
    async addDynamicCache(dynamic_id, host_mid, groupId, dynamic_type = '') {
        let cache = await this.getQuery('SELECT * FROM DynamicCaches WHERE dynamic_id = ? AND host_mid = ? AND groupId = ?', [dynamic_id, host_mid, groupId]);
        if (!cache) {
            const now = new Date().toISOString();
            const result = await this.runQuery('INSERT INTO DynamicCaches (dynamic_id, host_mid, groupId, dynamic_type, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?)', [dynamic_id, host_mid, groupId, dynamic_type, now, now]);
            cache = {
                id: result.lastID,
                dynamic_id,
                host_mid,
    ...
    async isDynamicPushed(dynamic_id, host_mid, groupId) {
        const result = await this.getQuery('SELECT COUNT(*) as count FROM DynamicCaches WHERE dynamic_id = ? AND host_mid = ? AND groupId = ?', [dynamic_id, host_mid, groupId]);
    ...
    async isSubscribed(host_mid, groupId) {
        const result = await this.getQuery('SELECT COUNT(*) as count FROM GroupUserSubscriptions WHERE host_mid = ? AND groupId = ?', [host_mid, groupId]);
        return (result?.count || 0) > 0;
    }
    /**
     * 批量同步配置文件中的订阅到数据库
    ...
    async addFilterWord(host_mid, word) {
        ...
        if (!filterWord) {
            const now = new Date().toISOString();
            const result = await this.runQuery('INSERT INTO FilterWords (host_mid, bilibiliUserHostMid, word, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?)', [host_mid, host_mid, word, now, now]);
            filterWord = {
                id: result.lastID,
                host_mid,
    ...
    async addFilterTag(host_mid, tag) {
        ...
        if (!filterTag) {
            const now = new Date().toISOString();
            const result = await this.runQuery('INSERT INTO FilterTags (host_mid, bilibiliUserHostMid, tag, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?)', [host_mid, host_mid, tag, now, now]);
            filterTag = {
                id: result.lastID,
                host_mid,
                bilibiliUserHostMid: host_mid,
    ...
    async extractTextAndTags(dynamicData) {
        ...
        if (liveContent) {
            const content = JSON.parse(liveContent);
    ...
    async cleanOldDynamicCache(days = 7) {
        ...
        const result = await this.runQuery('DELETE FROM DynamicCaches WHERE datetime(createdAt) < datetime(?)', [cutoffDate.toISOString()]);
        return result.changes ?? 0;
    }
