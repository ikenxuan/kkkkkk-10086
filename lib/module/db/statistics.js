...
export class StatisticsDBBase {
     ...
     */
    async recordMediaMetrics(groupId, platform, records, outcome, processingMs) {
        if (!isStatisticsPlatform(platform))
            return;
        // 一条媒体都没有、也没有耗时可记时不建行：否则每次纯图文解析都会
        // 在表里留一行全 0，把「这个平台有没有媒体数据」这个判断搞糊
        if (records.length === 0 && processingMs === undefined)
            return;
        let videoCount = 0;
        let audioCount = 0;
        ...
        for (const record of records) {
            ...
            if (record.durationMs !== undefined) {
                durationSamples += 1;
                if (record.kind === 'audio')
                    audioDurationMs += record.durationMs;
                else
                    videoDurationMs += record.durationMs;
                if (record.durationMs > maxDurationMs)
        ...
        await this.runQuery(`INSERT INTO MediaMetrics (
    ...
    async getGroupMediaMetrics(groupId) {
        return await this.allQuery('SELECT * FROM MediaMetrics WHERE groupId = ? ORDER BY platform', [groupId]);
    }
    /**
     ...
     */
    async getAllMediaMetrics() {
        return await this.allQuery('SELECT * FROM MediaMetrics ORDER BY groupId, platform');
    }
    async getGroupMediaSummary(groupId) {
        return summarizeMediaMetrics(await this.getGroupMediaMetrics(groupId));
    }
    async getGlobalMediaSummary() {
        return summarizeMediaMetrics(await this.getAllMediaMetrics());
    }
}
/** 分母为 0 时返回 undefined 而不是 0 —— 「没有数据」和「平均 0」在卡片上必须能区分 */
const average = (total, samples) => samples > 0 ? Math.round(total / samples) : undefined;
/**
 * 导出是为了让读取端（apps/statistics.ts）和测试都用同一套口径，
 * 避免两边各写一遍平均值公式后悄悄漂移。
 */
export const summarizeMediaMetrics = (rows) => {
    const platforms = {
    ...
    for (const row of rows) {
        if (!isStatisticsPlatform(row.platform))
            continue;
        ...
        if (row.maxDurationMs > maxDurationMs)
        ...
        if (row.maxDurationMs > (platform.maxDurationMs ?? 0))
            platform.maxDurationMs = row.maxDurationMs;
    }
