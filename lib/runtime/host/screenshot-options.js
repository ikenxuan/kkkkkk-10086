...
const isBareImagePayload = (value) => typeof value === 'string' || value instanceof Uint8Array;
/**
 ...
 * 越过这条线之后内部一律按消息段处理，`imagePayload` 的读写、`imageSlicer` 的分片
 * 都已经是照着消息段形状写的（`getImagePayload` 认 `file`），
 * 不必让四十多个 `Render()` 调用点各自记得包一次。
 *
 * 已经是消息段的（别的渲染器可能直接返回段）原样放过。
 */
export const ensureImageSegment = (image) => {
    if (!isBareImagePayload(image))
        return image;
    ...
    if (typeof toImageSegment !== 'function')
        return image;
