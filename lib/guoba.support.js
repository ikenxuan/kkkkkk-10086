...
const isConfigName = (name) => CONFIG_NAMES.some(configName => configName === name);
/**
 * 面板分组的先后顺序。
 *
                ...
                try {
                    ...
                    for (const [key, value] of entries) {
                        ...
                        if (!key.includes('.') && isPlainRecord(value)) {
                            if (!isConfigName(key))
                                throw new Error(`未知配置域: ${key}`);
                            if (!Config.ModifyPro(key, value))
                                throw new Error(`配置写入失败: ${key}`);
                            touched.add(key);
                            continue;
                        }
                        ...
                        if (!filename || parts.length === 0)
                            continue;
                        if (!isConfigName(filename))
                            throw new Error(`未知配置域: ${filename}`);
