...
const files = fs
	.readdirSync('./plugins/kkkkkk-10086/apps')
	.filter((file) => file.endsWith('.js'))

async function configfile() {
	const configPath = process.cwd() + '/plugins/kkkkkk-10086/config/config.json'
	const configExampleFile =
		process.cwd() + '/plugins/kkkkkk-10086/config/config.example.json'
	fs.access(configPath, fs.constants.F_OK, (err) => {
		if (err) {
			fs.copyFile(configExampleFile, configPath, (err) => {
				...
				logger.info(
					logger.green(
						'检测到kkkkkk-10086配置文件不存在，已自动创建 config.json created success'
					)
				)
			})
		}
